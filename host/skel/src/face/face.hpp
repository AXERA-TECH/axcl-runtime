...
namespace axcl::skel {
#define FACE_CLASS_NUM 1
#define FACE_DEFAULT_IO_DEPTH 4
#define FACE_MAX_IO_DEPTH 8
#define FACE_DEFAULT_POST_THREAD_NUMS 1

typedef struct axclSKEL_PRIV_FACE_PARAM_T {
    std::array<AXCL_SKEL_FILTER_CONFIG_T, FACE_CLASS_NUM> stFilterArr;

    axclSKEL_PRIV_FACE_PARAM_T() {
        memset(&stFilterArr, 0x00, sizeof(stFilterArr));
    }
} AXCL_SKEL_PRIV_FACE_PARAM_T;

///
class CFACE : public CSKEL {
public:
    CFACE(AXCL_SKEL_CTX_T ctx);
    virtual ~CFACE(AX_VOID) = default;
...
public:
    ...
    virtual AX_BOOL ObjectFliterProcess(const AX_CHAR *pstrObjectCategory,
                                                ...
                                                const std::array<AXCL_SKEL_FILTER_CONFIG_T, FACE_CLASS_NUM> &stFilterArr,
                                                const vector<float> &tlwh,
                                                const AX_U32 &nObjectCount);
    virtual AX_S32 SetPrivParam(AX_VOID);

private:
    AXCL_SKEL_PRIV_CTX_T m_stFaceCtx{0};
    AXCL_SKEL_PRIV_CTX_T m_stFaceAttrCtx{0};
    AXCL_SKEL_PRIV_FACE_PARAM_T m_sPrivFaceParam;
};
}
