...
AX_U64 CTrack::static_track_id_dict = 0;

CTrack::CTrack(const vector<float>& tlwh_,
                const float& score,
                const AX_U32& cls_id,
...
void CTrack::initTrackIDDict(const AX_U32 n_classes) {
#if 0
...
void CTrack::activate(KalmanFilter& kalman_filter, AX_U64 frame_id, AX_U64 real_frame_id) {
...
void CTrack::reActivate(CTrack& new_track, AX_U64 frame_id, AX_U64 real_frame_id, bool new_id) {
...
void CTrack::update(CTrack& new_track, AX_U64 frame_id, AX_U64 real_frame_id) {
    this->frame_id = frame_id;
    this->tracklet_len++;
...
void CTrack::staticTLBR() {  // x1y1wh -> x1y1x2y2
    tlbr.clear();
...
vector<float> CTrack::tlwhToxyah(vector<float> tlwh_tmp) {
...
vector<float> CTrack::toXYAH() {
    return tlwhToxyah(tlwh);
}

vector<float> CTrack::tlbrTotlwh(vector<float>& tlbr) {  // x1y1x2y2 -> x1y1wh
    tlbr[2] -= tlbr[0];                                 // w = x2 - x1
    tlbr[3] -= tlbr[1];
    return tlbr;
}

void CTrack::markLost() {
    state = TrackState::Lost;
}

void CTrack::markRemoved() {
    state = TrackState::Removed;
}

AX_U64 CTrack::nextID(const AX_U32& cls_id) {
#if 0
...
AX_U64 CTrack::endFrame() {
    return this->frame_id;
}

void CTrack::multiPredict(vector<CTrack*>& tracks, KalmanFilter& kalman_filter) {
