...
AX_S32 npu::AX_ENGINE_CreateContext(AX_ENGINE_HANDLE handle) {
    constexpr auto api(axcl::pkg::native::ENGINE_API::AX_ENGINE_CreateContext);
...
AX_S32 npu::AX_ENGINE_CreateContextV2(AX_ENGINE_HANDLE nHandle, AX_ENGINE_CONTEXT_T* pContext) {
    constexpr auto api(axcl::pkg::native::ENGINE_API::AX_ENGINE_CreateContextV2);
    ...
    int32_t ret;
    if (!m_rsp.unpack(api, &ret, pContext)) {
        LOG_MM_E(TAG, RSP_UNP_FAIL);
        return AXCL_DEF_ENGINE_ERR(AXCL_ERR_DECODE);
    }
    LOG_MM_D(TAG, RSP_UNP_CODE, ret);
