...
class axclrtEvnetTest : public testing::Test {
protected:
...
TEST_F(axclrtEvnetTest, Case01_axclrtCreateAndDestoryEvent) {

    axclrtEvent event;
    axclrtStream stream1;
    axclrtStream stream2;

    ASSERT_EQ(AXCL_SUCC, axclrtCreateEvent(&event));

    ASSERT_EQ(AXCL_SUCC, axclrtCreateStream(&stream1));
    ASSERT_EQ(AXCL_SUCC, axclrtCreateStream(&stream2));


    ASSERT_EQ(AXCL_SUCC, axclrtRecordEvent(event, stream1));
    ASSERT_EQ(AXCL_SUCC, axclrtStreamWaitEvent(stream2, event));
    ASSERT_EQ(AXCL_SUCC, axclrtStreamWaitEventWithTimeout(stream2, event, 999));

    ASSERT_EQ(AXCL_SUCC, axclrtSynchronizeStream(stream1));
    ASSERT_EQ(AXCL_SUCC, axclrtSynchronizeStream(stream2));

    EXPECT_EQ(AXCL_SUCC, axclrtDestroyStream(stream1));
    EXPECT_EQ(AXCL_SUCC, axclrtDestroyStream(stream2));

    ASSERT_EQ(AXCL_SUCC, axclrtDestroyEvent(event));
}