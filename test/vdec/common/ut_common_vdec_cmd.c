...
AX_S32 utOpenTestHooks(const SAMPLE_VDEC_CONTEXT_T *pstVdecCtx)
{
    char* file_name = NULL;
...
AX_S32 utVdecParseStreamCfg(const char *streamcfg, SAMPLE_VDEC_CONTEXT_T *pstVdecCtx, AX_VDEC_GRP VdGrp)
{
    int ret;
    int i;
    char *p;
    ...
    ret = utVdecCmdLineParseAndCheck(pstVdecCtx->argc, pstVdecCtx->argv,
                                   &pstVdecCtx->tCmdParam, VdGrp, 0);
    if (ret) {
        SAMPLE_CRIT_LOG("VdGrp=%d, utVdecCmdLineParseAndCheck ret:%d", VdGrp, ret);
        s32Ret = -1;
...
static void __PrintHelp_Com()
{
    ...
    printf("  -H:       max outbut buffer height. (for pool GetPicBufferSize), default: 1088\n");
    ...
    printf("  --uOutputFifoDepth:   output fifo depth. (0-34), default: 5\n");
    ...
    printf("  --bGetGrpAttr:        whether get group attr when decode. 1: enable. 0: disable. default: 0\n");
    printf("  --bGetChnAttr:        whether get channel attr when decode. 1: enable. 0: disable. default: 0\n");
    ...
    printf("  --enVideoMode:        Video Mode. 0: VIDEO_DEC_MODE_IPB. 1: VIDEO_DEC_MODE_IP. 2: VIDEO_DEC_MODE_I. 3: VIDEO_DEC_MODE_GDR. default: 0\n");
    printf("  --bResetTest          whether do reset test. (1: enable. 0: disable. default: 0\n");
    printf("  --bChnSwitchTest      whether do channel switch test. (1: enable. 0: disable. default: 0\n");
    printf("  --uResetCase           reset case num. (default: 0\n");
    printf("  --sResetTimes          reset operation times. (default: 1\n");
    printf("  --sResetPerFrmNum      Indicates the number of frames sent before the reset operation. (default: 100\n");
    printf("  --bMonitorFps         whether monitor fps. (1: enable. 0: disable. default: 0\n");
    printf("  --bCheckFrmParam      whether check frame param, such as u64PTS, u64SeqNum and u32TimeRef. (1: enable. 0: disable. default: 0\n");
    printf("  --bOutProcInfo        Whether to output proc information after decoding. (1: enable. 0: disable. default: 0\n");
    printf("  --fpsDeviation        fps deviation valid value. (default: 5. [0, 100])\n");
    printf("  --bMC:                whether used MultiCore to decode. 1: enable. 0: disable. default: 0\n");
    ...
    printf("  --bCheckLeftPic:      Whether to check the leftPic validity. Check only when select mode is 0. default: 0\n");
    ...
    printf("  --bGrpUnExist:        Whether to perform a GetFrame test after the decoder is destroyed. 1: enable. 0: disable. default: 0\n");
}
...
static AX_S32 __SampleInitOptions(SAMPLE_OPTION_T **ppOptions, SAMPLE_OPTION_NAME_T **ppOptName)
{
    ...
    if (ret) goto ERR_RET_LOG;
    ret = SampleOptionsFill(pOptions, i++, "bSendUserdata", '0', AX_TRUE);
    if (ret) goto ERR_RET_LOG;
    ret = SampleOptionsFill(pOptions, i++, "bQuitWait", 'q', AX_TRUE);
    if (ret) goto ERR_RET_LOG;
    ret = SampleOptionsFill(pOptions, i++, "sMilliSec", '0', AX_TRUE);
    if (ret) goto ERR_RET_LOG;
    ret = SampleOptionsFill(pOptions, i++, "sTestCnt", '0', AX_TRUE);
    if (ret) goto ERR_RET_LOG;
    ...
    if (ret) goto ERR_RET_LOG;
    ret = SampleOptionsFill(pOptions, i++, "bGetGrpAttr", '0', AX_TRUE);
    if (ret) goto ERR_RET_LOG;
    ret = SampleOptionsFill(pOptions, i++, "bGetChnAttr", '0', AX_TRUE);
    if (ret) goto ERR_RET_LOG;
    ...
    if (ret) goto ERR_RET_LOG;
    ret = SampleOptionsFill(pOptions, i++, "bMC", '0', AX_TRUE);
    if (ret) goto ERR_RET_LOG;
    ...
    if (ret) goto ERR_RET_LOG;
    ret = SampleOptionsFill(pOptions, i++, "bResetTest", '0', AX_TRUE);
    if (ret) goto ERR_RET_LOG;
    ret = SampleOptionsFill(pOptions, i++, "bChnSwitchTest", '0', AX_TRUE);
    if (ret) goto ERR_RET_LOG;
    ret = SampleOptionsFill(pOptions, i++, "uResetCase", '0', AX_TRUE);
    if (ret) goto ERR_RET_LOG;
    ret = SampleOptionsFill(pOptions, i++, "sResetTimes", '0', AX_TRUE);
    if (ret) goto ERR_RET_LOG;
    ret = SampleOptionsFill(pOptions, i++, "sResetPerFrmNum", '0', AX_TRUE);
    if (ret) goto ERR_RET_LOG;
    ret = SampleOptionsFill(pOptions, i++, "bMonitorFps", '0', AX_TRUE);
    if (ret) goto ERR_RET_LOG;
    ret = SampleOptionsFill(pOptions, i++, "bCheckFrmParam", '0', AX_TRUE);
    if (ret) goto ERR_RET_LOG;
    ret = SampleOptionsFill(pOptions, i++, "bOutProcInfo", '0', AX_TRUE);
    if (ret) goto ERR_RET_LOG;
    ret = SampleOptionsFill(pOptions, i++, "fpsDeviation", '0', AX_TRUE);
    if (ret) goto ERR_RET_LOG;
    ret = SampleOptionsFill(pOptions, i++, "enSkipFrame", '0', AX_TRUE);
    if (ret) goto ERR_RET_LOG;
    ret = SampleOptionsFill(pOptions, i++, "s32VdecVirtChn", '0', AX_TRUE);
    if (ret) goto ERR_RET_LOG;
    ret = SampleOptionsFill(pOptions, i++, "bCheckLeftPic", '0', AX_TRUE);
    if (ret) goto ERR_RET_LOG;
    ret = SampleOptionsFill(pOptions, i++, "bModifyFrmSize", '0', AX_TRUE);
    if (ret) goto ERR_RET_LOG;
    ret = SampleOptionsFill(pOptions, i++, "bCreatGrpEx", '0', AX_TRUE);
    if (ret) goto ERR_RET_LOG;
    ret = SampleOptionsFill(pOptions, i++, "bGrpUnExist", '0', AX_TRUE);
    if (ret) goto ERR_RET_LOG;
    ...
    for (j = 0; j < AX_VDEC_MAX_GRP_NUM; j++) {
        // FIXME: this comparison always false
        //if ((pOptName + j) == NULL) {
...
static AX_S32 __VdecCmdParaPrint(SAMPLE_VDEC_CMD_PARAM_T *pstPara, AX_VDEC_GRP VdGrp)
{
    ...
    SAMPLE_LOG("pstPara->McEnable:%d", pstPara->McEnable);

    SAMPLE_NOTICE_LOG("pstPara->bSendUserdata:%d", pstPara->bSendUserdata);
    SAMPLE_NOTICE_LOG("pstPara->bGetRbInfo:%d", pstPara->bGetRbInfo);
    SAMPLE_NOTICE_LOG("pstPara->bGetVuiParam:%d", pstPara->bGetVuiParam);
...
static AX_S32 __VdecParameterCheck(SAMPLE_VDEC_CMD_PARAM_T *pCml, AX_VDEC_GRP VdGrp)
{
    ...
    if (pstCmd->bMonitorFps) {
        if (pstCmd->fpsDeviation < 0 || pstCmd->fpsDeviation > 100) {
            SAMPLE_CRIT_LOG("fpsDeviation:%d out ouf range[0, 100]\n", pstCmd->fpsDeviation);
            ret = -1;
            goto ERR_RET;
        }
    }
    ...
    if (pstCmd->bCreatGrpEx) {
        if (pstCmd->enSelectMode != AX_VDEC_SELECT_MODE_PRIVATE) {
            SAMPLE_CRIT_LOG("enSelectMode:%d not support CreatGrpEx\n", pstCmd->enSelectMode);
            ret = -1;
            goto ERR_RET;
        }
        if (pstCmd->bChnSwitchTest) {
            SAMPLE_CRIT_LOG("bChnSwitchTest:%d not support CreatGrpEx\n", pstCmd->bChnSwitchTest);
            ret = -1;
            goto ERR_RET;
        }
    }
...
AX_S32 utVdecCmdLineParseAndCheck(AX_S32 argc, AX_CHAR **argv, SAMPLE_VDEC_CMD_PARAM_T *pstPara,
                                AX_VDEC_GRP VdGrp, AX_BOOL bLink)
{
    ...
    while ((s32Ret = SampleGetOption(argc, argv, options, &prm)) != -1) {
        ...
        case '0': {
            ...
            else if (strcmp(pPrm->longOpt, "bChnSwitchTest") == 0) {
                pstCmd->bChnSwitchTest = atoi(optarg);
                pstCmd->enSelectMode = AX_VDEC_RECV_MODE_AFTER_SEND;
                pstCmd->tChnCfg[0].bChnEnable = AX_TRUE;
                pstCmd->tChnCfg[0].VdChn = 0;
                pstCmd->tChnCfg[1].bChnEnable = AX_TRUE;
                pstCmd->tChnCfg[1].VdChn = 1;
                pstCmd->tChnCfg[2].bChnEnable = AX_TRUE;
                pstCmd->tChnCfg[2].VdChn = 2;
            }
            else if (strcmp(pPrm->longOpt, "sTestCnt") == 0) {
                pstCmd->sTestCnt = atoi(optarg);
            }
            ...
            else if (strcmp(pPrm->longOpt, "bSendUserdata") == 0) {
               pstCmd->bSendUserdata = atoi(optarg);
            }
            ...
            else if (strcmp(pPrm->longOpt, "bGetGrpAttr") == 0) {
               pstCmd->bGetGrpAttr = atoi(optarg);
            }
            else if (strcmp(pPrm->longOpt, "bGetChnAttr") == 0) {
               pstCmd->bGetChnAttr = atoi(optarg);
            }
                ...
                else if (pstCmd->enSelectMode == AX_VDEC_RECV_MODE_AFTER_SEND) {
                    pstCmd->sMilliSec = -1;
                }
            }
            ...
            else if (strcmp(pPrm->longOpt, "bMC") == 0) {
                pstCmd->McEnable = atoi(optarg);
            }
            ...
            else if (strcmp(pPrm->longOpt, "bCheckLeftPic") == 0) {
                pstCmd->bCheckLeftPic = atoi(optarg);
            }
            ...
            else if (strcmp(pPrm->longOpt, "bResetTest") == 0) {
                pstCmd->bResetTest = atoi(optarg);
            }
            else if (strcmp(pPrm->longOpt, "uResetCase") == 0) {
                pstCmd->uResetCase = atoi(optarg);
            }
            else if (strcmp(pPrm->longOpt, "sResetTimes") == 0) {
                pstCmd->sResetTimes = atoi(optarg);
            }
            else if (strcmp(pPrm->longOpt, "sResetPerFrmNum") == 0) {
                pstCmd->sResetPerFrmNum = atoi(optarg);
            }
            else if (strcmp(pPrm->longOpt, "bMonitorFps") == 0) {
                pstCmd->bMonitorFps = atoi(optarg);
            }
            else if (strcmp(pPrm->longOpt, "bCheckFrmParam") == 0) {
                pstCmd->bCheckFrmParam = atoi(optarg);
            }
            else if (strcmp(pPrm->longOpt, "bOutProcInfo") == 0) {
                pstCmd->bOutProcInfo = atoi(optarg);
            }
            else if (strcmp(pPrm->longOpt, "fpsDeviation") == 0) {
                pstCmd->fpsDeviation = atoi(optarg);
            }
            ...
            else if (strcmp(pPrm->longOpt, "bGrpUnExist") == 0) {
                pstCmd->bGrpUnExist = atoi(optarg);
            }
            else {
...
AX_S32 utVdecDefaultParamsSet(SAMPLE_VDEC_CMD_PARAM_T *pstCmdPara)
{
    ...
    pstCmd->sResetPerFrmNum = 100;
    pstCmd->uResetCase = 0;
    pstCmd->sResetTimes = 1;
    pstCmd->fpsDeviation = 5;
