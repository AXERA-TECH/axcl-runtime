...
#define SAMPLE_VDEC_OUTPUT_FRAMEBUF_CNT     (5)
...
#define AX_VDEC_PRIVATE_DATA_VERIFY_NUM     0xAFAF5A5A
#define AX_VDEC_USER_DATA_VERIFY_NUM        0x5ABEEF
...
typedef struct axSAMPLE_VDEC_USERPIC_T {
    ...
    AX_VOID *pHostVirAddr[AX_DEC_MAX_CHN_NUM];
...
typedef enum {
    ...
    AX_VDEC_RECV_MODE_AFTER_SEND,
    AX_VDEC_SELECT_MODE_POSIX,
    AX_VDEC_RECV_MODE_BUTT
} AX_VDEC_RECV_MODE_E;
...
typedef enum {
    AX_VDEC_RESET_CASE_NORMAL = 0,
    AX_VDEC_RESET_CASE_FORCE_BLOCK,
    AX_VDEC_RESET_CASE_BUTT
} AX_VDEC_RESET_CASE_E;
...
typedef struct axSAMPLE_VDEC_CMD_PARAM_T {
    ...
    AX_BOOL bOutProcInfo;
    AX_U32  fpsDeviation;
    AX_BOOL bResetTest;
    AX_BOOL bChnSwitchTest;
    AX_U32  uResetCase;
    AX_S32  sResetTimes;
    AX_S32  sResetPerFrmNum;
    AX_BOOL enSkipFrame;
    AX_S32 s32VdecVirtChn;
    AX_BOOL bCheckLeftPic;
...
typedef struct axSAMPLE_VDEC_TEST_CTRL_T {
    AX_S32              s32TestCnt;
} SAMPLE_VDEC_TEST_CTRL_T;

typedef struct SAMPLE_VDEC_CONTEXT_T {
    ...
    /* H264/H265: Multicore enable or not */
    AX_BOOL bVdecMc;
    AX_S32 oneShot;
    ...
    SAMPLE_VDEC_TEST_CTRL_T stVdecTestCtrl;
...
#define SAMPLE_ERR_LOG(str, arg...)  do { \
        printf("[SAMPLE][AX_VDEC][tid:%ld][ERROR][%s][line:%d]"str"\n", \
                gettid(), __func__, __LINE__, ##arg); \
    } while (0)

#define SAMPLE_CRIT_LOG(str, arg...)        do{ \
        printf("[SAMPLE][AX_VDEC][tid:%ld][CR_ERROR][%s][line:%d]"str"\n", \
                gettid(), __func__, __LINE__, ##arg); \
    } while (0)
...
#define SAMPLE_INFO_LOG(str, arg...)  do { \
        printf("[SAMPLE][AX_VDEC][tid:%ld][T][%s][line:%d]: "str"\n", \
               gettid(), __func__, __LINE__, ##arg); \
    } while (0)
...
#define SAMPLE_LOG_NN SAMPLE_LOG_N
