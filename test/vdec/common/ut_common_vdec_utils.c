...
{
    int findSPS = AX_ERR_VDEC_UNKNOWN;
    if (buf == NULL) {
    ...
    if (buf[2] == 0) {
        if ((buf[4] & 0x1f) == 7) { //start code 00 00 00 01
            len -= 5;
            buf += 5;
            findSPS = AX_SUCCESS;
        }
        ...
        if ((buf[0] & 0x1f) == 7) { //no start code, 0x67 ?
            len -= 1;
            buf += 1;
            findSPS = AX_SUCCESS;
        }
    ...
    if (findSPS != AX_SUCCESS) {
        SAMPLE_LOG("findSPS failed! Failed to parse width and height.\n"
...
int OutputFileCheckMD5(AX_VDEC_GRP VdGrp, AX_VDEC_CHN VdChn, const AX_VIDEO_FRAME_INFO_T *frameInfo, char *md5_str)
{
    ...
    if ((frameInfo->stVFrame.u64PhyAddr[0] == 0) || (frameInfo->stVFrame.u32PicStride[0] == 0)) {
        SAMPLE_CRIT_LOG("VdGrp=%d, VdChn=%d, stVFrame.u64PhyAddr[0]:0x%llx stVFrame.u32PicStride[0]:%d\n",
                       VdGrp, VdChn, frameInfo->stVFrame.u64PhyAddr[0], frameInfo->stVFrame.u32PicStride[0]);
        s32Ret = 0;
        goto ERR_RET;
    }
    ...
    pLumaVirAddr = AXCL_SYS_Mmap(frameInfo->stVFrame.u64PhyAddr[0], lumaMapSize);
    if (!pLumaVirAddr) {
        s32Ret = AX_ERR_VDEC_UNKNOWN;
        SAMPLE_CRIT_LOG("AXCL_SYS_Mmap luma FAILED\n");
        goto ERR_RET;
    }

    SAMPLE_LOG("AXCL_SYS_Mmap luma success, pLumaVirAddr=%p,lumaMapSize=%d\n",
               pLumaVirAddr, lumaMapSize);

    chromaMapSize = frameInfo->stVFrame.u32PicStride[0] * SIZE_ALIGN(frameInfo->stVFrame.u32Height, 16) / 2;
    pChromaVirAddr = AXCL_SYS_Mmap(frameInfo->stVFrame.u64PhyAddr[1], chromaMapSize);
    if (!pChromaVirAddr) {
        s32Ret = AX_ERR_VDEC_UNKNOWN;
        SAMPLE_CRIT_LOG("AXCL_SYS_Mmap chroma FAILED\n");
        goto ERR_RET_CHROMA;
    }

    SAMPLE_LOG("AXCL_SYS_Mmap chroma success, pChromaVirAddr=%p,chromaMapSize=%d\n",
...
ERR_RET_LUMA:
    if (pLumaVirAddr) {
        sRet = AXCL_SYS_Munmap(pLumaVirAddr, lumaMapSize);
        if (sRet) {
            s32Ret = sRet;
            SAMPLE_CRIT_LOG("AXCL_SYS_Munmap luma FAILED,sRet=0x%x\n", sRet);
        } else {
            SAMPLE_LOG("AXCL_SYS_Munmap luma success,pLumaVirAddr=%p,lumaMapSize=%d\n",
                      pLumaVirAddr, lumaMapSize);
        }
    }
ERR_RET_CHROMA:
    if (pChromaVirAddr) {
        sRet = AXCL_SYS_Munmap(pChromaVirAddr, chromaMapSize);
        if (sRet) {
            s32Ret = sRet;
            SAMPLE_CRIT_LOG("AXCL_SYS_Munmap chroma FAILED,sRet=0x%x\n", sRet);
        } else {
            SAMPLE_LOG("AXCL_SYS_Munmap chroma success,pChromaVirAddr=%p,chromaMapSize=%d\n",
                      pChromaVirAddr, chromaMapSize);
        }
    }
ERR_RET:
...
AX_S32 LoadOneFileToMem(const AX_CHAR *ps8File, AX_U8 **ppu8Mem, size_t *pLen)
{
    ...
    *ppu8Mem = (AX_U8 *)malloc(tmp_size);
    if (*ppu8Mem == NULL) {
        SAMPLE_CRIT_LOG("malloc tmp_size:0x%lx FAILED!", tmp_size);
        res = AX_ERR_VDEC_UNKNOWN;
        goto ERR_RET_CLOSE;
    }
    ...
    if (tmp_size != read_size) {
        SAMPLE_CRIT_LOG("fread FAILED! tmp_size:0x%lx != read_size:0x%lx\n", tmp_size, read_size);
        res = AX_ERR_VDEC_UNKNOWN;
        goto ERR_RET_FREE;
    }
...
static off_t __FindFileNextStartCode(const SAMPLE_INPUT_FILE_INFO_T *pstBsInfo, AX_U32 *puZeroCount)
{
    AX_S32 i;
    off_t sStart;
    off_t oFileStart;
    ...
    *puZeroCount = 0;
    ...
    while (1) {
        ...
        read_len = fread(tmp_buf, 1, uFileReadLen, pstBsInfo->fInput);
        if (read_len != uFileReadLen) {
            SAMPLE_CRIT_LOG("fread FAILED! read_len:0x%x, uFileReadLen:0x%x", read_len, uFileReadLen);
        }
                                         ...
                                         size_t oStreamLen, size_t *pReadLen)
{
    ...
    if (uBufSize < oStreamLen) {
        SAMPLE_CRIT_LOG("uBufSize:0x%x < oStreamLen:0x%lx", uBufSize, oStreamLen);
        sRet = AX_ERR_VDEC_UNKNOWN;
        goto ERR_RET;
    }
    ...
    *pReadLen = 0;
    ...
    if ((oOffset + oStreamLen) < uBufSize) {
        ...
        if (sReadLen != oStreamLen) {
            SAMPLE_CRIT_LOG("fread FAILED! sReadLen:0x%lx != oStreamLen:0x%lx", sReadLen, oStreamLen);
            sRet = AX_ERR_VDEC_UNKNOWN;
            goto ERR_RET;
        }
        ...
        if (sReadLen != uBufSize - oOffset) {
            SAMPLE_CRIT_LOG("fread FAILED! sReadLen:0x%lx != (uBufSize:0x%x - oOffset:0x%lx):0x%x",
        ...
        if (tmp_len != left_len) {
            SAMPLE_CRIT_LOG("fread FAILED! tmp_len:0x%x != left_len:0x%x, oStreamLen:0x%lx uBufSize:0x%x oOffset:0x%lx",
...
AX_S32 StreamFileParserReadFrame(const SAMPLE_INPUT_FILE_INFO_T *pstBsInfo, SAMPLE_STREAM_BUF_T *pstStreamBuf, size_t *pReadLen)
{
    ...
    oBegin = __FindFileNextStartCode(pstBsInfo, &uZeroCount);
    ...
    oEnd = oNalBegin = __FindFileNextStartCode(pstBsInfo, &uZeroCount);
    ...
    if (enBoundary != BSPARSER_BOUNDARY_NON_SLICE_NAL) {
        while (1) {
            ...
            if (enBoundary == BSPARSER_NO_BOUNDARY) {
                oNalBegin = __FindFileNextStartCode(pstBsInfo, &uZeroCount);
            }
    ...
    uBufSize = pstStreamBuf->uBufSize;
    if (uBufSize < oStreamLen) {
        SAMPLE_CRIT_LOG("uBufSize:0x%x < oStreamLen:0x%lx. bufSize is not enough, please increase STREAM_BUFFER_MAX_SIZE",
                        uBufSize, oStreamLen);
        sRet = AX_ERR_VDEC_UNKNOWN;
        goto ERR_RET;
    }
...
ret:
    ...
    if(0 == s32Ret) {
        ...
        if (pstStreamBuf->bRingbuf == AX_TRUE) {
            ...
            if (sReadLen != imgLen) {
                SAMPLE_CRIT_LOG("fread FAILED! sReadLen:0x%lx != imgLen:0x%x, pBufStart:%p, %ld, %ld, f:%p", sReadLen, imgLen, pBufStart, curPos, pstBsInfo->curPos, pstBsInfo->fInput);
                return -1;
            }
...
static AX_S32 __VdecUsrPicRead(FILE *pFileIn, AX_VIDEO_FRAME_T *pstFrame, AX_VOID *pHostVirAddr)
{
    AX_S32 sRet = 0;
    ...
    case AX_FORMAT_YUV420_SEMIPLANAR_VU:
        ...
        for (i = 0; i < rows; i++) {
            ...
            if (realRead < widthSrc) {
                SAMPLE_CRIT_LOG("fread failed! realRead=%d <  widthSrc=%d\n", realRead, widthSrc);
                sRet = AX_ERR_VDEC_UNKNOWN;
                goto ERR_RET;
            }
            pVaddr += strideSrc;
...
static AX_S32 __VdecUsrPicInfoFill(SAMPLE_VDEC_USRPIC_ARGS_T *pstUsrPicArgs, SAMPLE_VDEC_USERPIC_T *pstVdecUserPic)
{
    ...
    for (VdChn = 0; VdChn < AX_DEC_MAX_CHN_NUM; VdChn++) {
        if (pstUsrPicArgs->tChnParam[VdChn].bChnEnable == AX_FALSE) continue;
        if (pstUsrPicArgs->tChnParam[VdChn].bUserPicEnable) {
            if ((AX_FORMAT_YUV420_SEMIPLANAR != pstUsrPicArgs->tChnParam[VdChn].enImgFormat) &&
                (AX_FORMAT_YUV420_SEMIPLANAR_VU != pstUsrPicArgs->tChnParam[VdChn].enImgFormat)) {
                SAMPLE_CRIT_LOG("VdGrp=%d, VdChn=%d, unsupport enImgFormat:%d\n",
                                VdGrp, VdChn, pstUsrPicArgs->tChnParam[VdChn].enImgFormat);
                sRet = AX_ERR_VDEC_UNKNOWN;
                goto ERR_RET;
            }
            uWidth = pstUsrPicArgs->tChnParam[VdChn].u32PicWidth;
            u32FrameStride = AX_COMM_ALIGN(uWidth * uPixBits, AX_VDEC_WIDTH_ALIGN * 8) / 8;;
            pstVdecUserPic->pUsrPicFilePath[VdChn] = pstUsrPicArgs->tChnParam[VdChn].pUsrPicFilePath;
            FrameSize = AX_VDEC_GetPicBufferSize(u32FrameStride,
                                                 pstUsrPicArgs->tChnParam[VdChn].u32PicHeight,
                                                 pstUsrPicArgs->tChnParam[VdChn].enImgFormat,
                                                 &tCompressInfo, pstUsrPicArgs->enDecType);
            ...
            pstVdecUserPic->PoolId[VdChn] = AXCL_POOL_CreatePool(&stPoolConfig);
            if (AX_INVALID_POOLID == pstVdecUserPic->PoolId[VdChn]) {
                SAMPLE_CRIT_LOG("VdGrp=%d, VdChn=%d, AXCL_POOL_CreatePool FAILED! BlkCnt:%d, BlkSize:0x%llx\n",
                                VdGrp, VdChn, stPoolConfig.BlkCnt, stPoolConfig.BlkSize);
                sRet = AX_ERR_VDEC_NOBUF;
                goto ERR_RET_DESTROY;
            }

            blkId = AXCL_POOL_GetBlock(pstVdecUserPic->PoolId[VdChn], FrameSize, NULL);
            if (AX_INVALID_BLOCKID == blkId) {
                SAMPLE_CRIT_LOG("VdGrp=%d, VdChn=%d, AXCL_POOL_GetBlock FAILED! PoolId:%d, BlkSize:0x%llx\n",
                                VdGrp, VdChn, pstVdecUserPic->PoolId[VdChn], stPoolConfig.BlkSize);
                sRet = AX_ERR_VDEC_NOBUF;
                goto ERR_RET_DESTROY;
            }

            sRet = axclrtMallocHost(&pVirAddr, FrameSize);
            if (AX_SUCCESS != sRet) {
            ...
            pstVdecUserPic->pHostVirAddr[VdChn] = pVirAddr;

            pstVdecUserPic->BlkId[VdChn] = blkId;
            pstUserPic = &pstVdecUserPic->stUserPic;
            pstUserPic->bEnable[VdChn] = AX_TRUE;
            pstUserPic->bInstant = pstUsrPicArgs->bUsrInstant;
            pstUserPic->stFrmInfo[VdChn].bEndOfStream = AX_TRUE;
            pstUserPic->stFrmInfo[VdChn].enModId = AX_ID_VDEC;
            pstUserPic->stFrmInfo[VdChn].stVFrame.u32BlkId[0] = blkId;
            pstUserPic->stFrmInfo[VdChn].stVFrame.u32FrameSize = FrameSize;
            pstUserPic->stFrmInfo[VdChn].stVFrame.u32Width = pstUsrPicArgs->tChnParam[VdChn].u32PicWidth;
            pstUserPic->stFrmInfo[VdChn].stVFrame.u32Height = pstUsrPicArgs->tChnParam[VdChn].u32PicHeight;
            pstUserPic->stFrmInfo[VdChn].stVFrame.enImgFormat = pstUsrPicArgs->tChnParam[VdChn].enImgFormat;
            pstUserPic->stFrmInfo[VdChn].stVFrame.enVscanFormat = AX_VSCAN_FORMAT_RASTER;
            pstUserPic->stFrmInfo[VdChn].stVFrame.stCompressInfo.enCompressMode = AX_COMPRESS_MODE_NONE;
            pstUserPic->stFrmInfo[VdChn].stVFrame.u64PhyAddr[0] = AXCL_POOL_Handle2PhysAddr(blkId);
            pstUserPic->stFrmInfo[VdChn].stVFrame.u64VirAddr[0] = (AX_U64)AXCL_POOL_GetBlockVirAddr(blkId);
            pstUserPic->stFrmInfo[VdChn].stVFrame.u32PicStride[0] = u32FrameStride;
            pstUserPic->stFrmInfo[VdChn].stVFrame.u64PhyAddr[1] = pstUserPic->stFrmInfo[VdChn].stVFrame.u64PhyAddr[0] +
                                                                  pstUserPic->stFrmInfo[VdChn].stVFrame.u32PicStride[0] *
                                                                  pstUserPic->stFrmInfo[VdChn].stVFrame.u32Height;
            pstUserPic->stFrmInfo[VdChn].stVFrame.u64PhyAddr[2] = 0;
            pstUserPic->stFrmInfo[VdChn].stVFrame.u64VirAddr[1] = pstUserPic->stFrmInfo[VdChn].stVFrame.u64VirAddr[0] +
                                                                  pstUserPic->stFrmInfo[VdChn].stVFrame.u32PicStride[0] *
                                                                  pstUserPic->stFrmInfo[VdChn].stVFrame.u32Height;
            pstUserPic->stFrmInfo[VdChn].stVFrame.u64VirAddr[2] = 0;
            pstUserPic->stFrmInfo[VdChn].stVFrame.u64PTS = 0;
        }
    }
    ...
    for (VdChn = 0; VdChn < AX_DEC_MAX_CHN_NUM; VdChn++) {
        ...
        if (pstUsrPicArgs->tChnParam[VdChn].bUserPicEnable) {
            uWidth = pstUserPic->stFrmInfo[VdChn].stVFrame.u32Width;
            pstVdecUserPic->fpUsrPic[VdChn] = fopen(pstVdecUserPic->pUsrPicFilePath[VdChn],"rb");
            if (pstVdecUserPic->fpUsrPic[VdChn] == NULL) {
                SAMPLE_CRIT_LOG("VdGrp=%d, VdChn=%d, can't open file %s in VDEC_PREPARE_USERPIC.\n",
                                VdGrp, VdChn, pstVdecUserPic->pUsrPicFilePath[VdChn]);
                sRet = AX_ERR_VDEC_UNKNOWN;
                goto ERR_RET_FREE;
            }

            fpUserYUV = pstVdecUserPic->fpUsrPic[VdChn];


            sRet = __VdecUsrPicRead(fpUserYUV, &pstUserPic->stFrmInfo[VdChn].stVFrame, pstVdecUserPic->pHostVirAddr[VdChn]);
            if (sRet) {
            ...
            sRet = axclrtMemcpy((AX_VOID *)AXCL_POOL_Handle2PhysAddr(blkId), pVirAddr, FrameSize, AXCL_MEMCPY_HOST_TO_DEVICE);
            if (AX_SUCCESS != sRet) {
                SAMPLE_CRIT_LOG("axclrtMemcpy FAILED! ret:0x%x\n", sRet);
                goto ERR_RET_HOST_FREE;
            }

            SAMPLE_LOG("VdGrp=%d, VdChn=%d, .u64PhyAddr[0]:%llx .u64VirAddr[0]:%llx .u64PhyAddr[1]:%llx .u64VirAddr[1]:%llx\n",
                       VdGrp, VdChn,
                       pstUserPic->stFrmInfo[VdChn].stVFrame.u64PhyAddr[0],
                       pstUserPic->stFrmInfo[VdChn].stVFrame.u64VirAddr[0],
                       pstUserPic->stFrmInfo[VdChn].stVFrame.u64PhyAddr[1],
                       pstUserPic->stFrmInfo[VdChn].stVFrame.u64VirAddr[1]);
        }
    }
...
ERR_RET_HOST_FREE:
    for (VdChn = 0; VdChn < AX_DEC_MAX_CHN_NUM; VdChn++) {
        if (pstUsrPicArgs->tChnParam[VdChn].bChnEnable == AX_FALSE) continue;
        if (pstUsrPicArgs->tChnParam[VdChn].bUserPicEnable) {
            axclrtFreeHost(pstVdecUserPic->pHostVirAddr[VdChn]);
        }
    }
ERR_RET_DESTROY:
    tmp_ci = VdChn;
    for (VdChn = 0; VdChn < tmp_ci; VdChn++) {
        if (pstUsrPicArgs->tChnParam[VdChn].bChnEnable == AX_FALSE) continue;
        if (pstUsrPicArgs->tChnParam[VdChn].bUserPicEnable) {
            if (pstVdecUserPic->BlkId[VdChn] != AX_INVALID_BLOCKID) {
                ret = AXCL_POOL_ReleaseBlock(pstVdecUserPic->BlkId[VdChn]);
                if (ret)
                    SAMPLE_CRIT_LOG("VdGrp=%d, VdChn=%d, PoolId[VdChn]:%d,  AXCL_POOL_ReleaseBlock FAILED! ret:0x%x %s",
                                    VdGrp, VdChn, pstVdecUserPic->BlkId[VdChn], ret, AX_VdecRetStr(ret));
            }
            if (pstVdecUserPic->PoolId[VdChn] != AX_INVALID_POOLID) {
                ret = AXCL_POOL_DestroyPool(pstVdecUserPic->PoolId[VdChn]);
                if (ret)
                    SAMPLE_CRIT_LOG("VdGrp=%d, VdChn=%d, PoolId[VdChn]:%d, AXCL_POOL_DestroyPool FAILED! ret:0x%x %s",
                                VdGrp, VdChn, pstVdecUserPic->PoolId[VdChn], ret, AX_VdecRetStr(ret));
            }
        }
    }
...
AX_S32 __VdecUsrPicCreat(SAMPLE_VDEC_USRPIC_ARGS_T *pstUsrPicArgs, SAMPLE_VDEC_USERPIC_T *pstVdecUserPic)
{
    ...
    for (VdChn = 0; VdChn < AX_DEC_MAX_CHN_NUM; VdChn++) {
        if (pstUsrPicArgs->tChnParam[VdChn].bChnEnable == AX_FALSE) continue;
        if (pstUsrPicArgs->tChnParam[VdChn].bUserPicEnable)
            pstVdecUserPic->usrPicChnEnaCnt++;
    }

    if (pstVdecUserPic->usrPicChnEnaCnt == 0)
        goto ERR_RET;
    ...
    s32Ret = AXCL_VDEC_DisableUserPic(VD_GRP_API_MGR_ID(VdGrp));
    if (s32Ret != AX_SUCCESS) {
        SAMPLE_CRIT_LOG("VdGrp=%d, AXCL_VDEC_DisableUserPic FAILED! ret:0x%x %s\n",
                        VdGrp, s32Ret, AX_VdecRetStr(s32Ret));
        goto ERR_RET;
    }

    s32Ret = AXCL_VDEC_SetUserPic(VD_GRP_API_MGR_ID(VdGrp), &pstVdecUserPic->stUserPic);
    if (s32Ret != AX_SUCCESS) {
        SAMPLE_CRIT_LOG("VdGrp=%d, AXCL_VDEC_SetUserPic FAILED! ret:0x%x %s\n",
                        VdGrp, s32Ret, AX_VdecRetStr(s32Ret));
        goto ERR_RET;
    }
                         ...
                         AX_BOOL *pContSendStm, SAMPLE_VDEC_CONTEXT_T *pstCtx)
{
    ...
    for (VdChn = 0; VdChn < AX_DEC_MAX_CHN_NUM; VdChn++) {
        if (pstVdecUserPic->stUserPic.bEnable[VdChn])
            cnt++;
    }
    ...
    if (cnt) {
        ...
        if (!pstVdecUserPic->stUserPic.bInstant)
            sleep(2);
        sRet = AXCL_VDEC_EnableUserPic(VD_GRP_API_MGR_ID(VdGrp));
        if (sRet) {
            SAMPLE_CRIT_LOG("VdGrp=%d, AXCL_VDEC_EnableUserPic FAILED! ret:0x%x %s",
                             VdGrp, sRet, AX_VdecRetStr(sRet));
            goto ERR_RET;
        }
...
AX_VOID VdecUserPicDestroy(AX_VDEC_GRP VdGrp, SAMPLE_VDEC_USERPIC_T *pstVdecUserPic)
{
    ...
    if (cnt) {
        ret = AXCL_VDEC_DisableUserPic(VD_GRP_API_MGR_ID(VdGrp));
        if (ret)
                SAMPLE_CRIT_LOG("VdGrp=%d, AXCL_VDEC_DisableUserPic FAILED! ret:0x%x %s",
...
int VdecCommonPoolPrintf(AX_VOID)
{
    ...
    s32Ret = AXCL_POOL_GetConfig(&PoolFloorPlan);
    if (s32Ret) {
        SAMPLE_CRIT_LOG("AXCL_POOL_SetConfig FAILED! 0x%x\n", s32Ret);
        goto ERR_RET;
    }
                            ...
                            SAMPLE_BITSTREAM_INFO_T *pstBitStreamInfo)
{
    ...
    pstFfmpeg->s32VideoIndex = AX_ERR_VDEC_UNKNOWN;
