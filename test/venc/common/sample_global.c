...
static void SampleExifInfoFill(AX_USER_EXIF_INFO_T *pstUserExifInfo, SAMPLE_VENC_SENDFRAME_PARA_T *pstArg)
{
    ...
    pstUserExifInfo->stExifGpsInfo.f32GpsLatitude = SampleConvertDegreesToDecimal(29, 11, 29);
    pstUserExifInfo->stExifGpsInfo.enGpsLongitudeRef = AX_VENC_EXIF_GPS_LONGITUDE_REF_EAST;
    pstUserExifInfo->stExifGpsInfo.f32GpsLongitude = SampleConvertDegreesToDecimal(118, 21, 29);
    ...
    strcpy((AX_CHAR *)pstUserExifInfo->stExifGpsInfo.u8GpsAreaInformation, "hangzhou");
...
AX_VOID *SAMPLE_VENC_SendFrameProc(AX_VOID *arg)
{
    ...
    if (AXCL_SUCC != ret) {
        SAMPLE_LOG_ERR("axcl create cxt err, ret=0x%x.\n", ret);
        return NULL;
    }
    ...
    while (pstArg->bSendFrmStart) {
        ...
        if (pstArg->stFbcInfo.fbcType) {
            readSize = COMMON_VENC_ReadFbcFile(fFileIn, strideY, height, &pstArg->stFbcInfo, &stFrame.stVFrame, host_mem);
        } else {
...
AX_VOID *SAMPLE_VENC_SendFramePerfProc(AX_VOID *arg)
{
    ...
    for (AX_S32 blk = 0; blk < pstArg->vbCnt; blk++) {
        ...
        /* read frame data from yuv file */
        readSize = COMMON_VENC_ReadFile(fFileIn, width, strideY, height, eFmt, (void *)host_mem);
    ...
    for (AX_S32 blk = pstArg->vbCnt - 1; blk >= 0; blk--) {
        if (AX_INVALID_BLOCKID != gBlkArr[blk])
            AXCL_POOL_ReleaseBlock(gBlkArr[blk]);
    }
...
AX_VOID *SAMPLE_VENC_SendFrameEx(AX_VOID *arg)
{
    ...
    AX_S8 *QpMapVirAddr, *QpMapVirAddrHost;
    ...
    /* malloc on host and memcpy to device */
    AX_VOID *host_mem = NULL;
    axclrtMallocHost(&host_mem, pCml->BlkSize);
    ...
    while (pstArg->bSendFrmStart) {
        ...
        stFrame.stUserFrame.stVFrame.u64PhyAddr[0] = AXCL_POOL_Handle2PhysAddr(blkId);
        stFrame.stUserFrame.stVFrame.u64VirAddr[0] = (AX_U64)AXCL_POOL_GetBlockVirAddr(blkId);
        ...
        if (bQpMapProc) {
            ...
            QpMapVirAddrHost = (AX_S8 *)(host_mem + frameSize);
        ...
        if (pstArg->stFbcInfo.fbcType) {
            readSize =
                COMMON_VENC_ReadFbcFile(fFileIn, strideY, height, &pstArg->stFbcInfo, &stFrame.stUserFrame.stVFrame, host_mem);
        } else {
            /* read frame data from yuv file */
            readSize = COMMON_VENC_ReadFile(fFileIn, width, strideY, height, eFmt,
                                            (void *)host_mem);
            ...
            if (bLoopEncode) {
                ...
                if (pstArg->stFbcInfo.fbcType) {
                    readSize = COMMON_VENC_ReadFbcFile(fFileIn, strideY, height, &pstArg->stFbcInfo,
                                                       &stFrame.stUserFrame.stVFrame, host_mem);
                } else {
                    /* read frame data from yuv file */
                    readSize = COMMON_VENC_ReadFile(fFileIn, width, strideY, height, eFmt,
                                                    (void *)host_mem);
        ...
        if (bQpMapProc) {
            memset(QpMapVirAddrHost, 0x0, qpmapSize);
            /* qpmap setting*/
            if (AX_VENC_QPMAP_QP_DELTA == qpmapQpType || AX_VENC_QPMAP_QP_ABS == qpmapQpType) {
                stFrame.stUserRcInfo.bQpMapValid = AX_TRUE;
                SampleCopyQPDelta2Memory(width, height, maxCuSize, qpmapBlkUnit, QpMapVirAddrHost, qpmapQpType);
            }
            ...
            if (AX_VENC_QPMAP_BLOCK_SKIP == qpmapBlkType || AX_VENC_QPMAP_BLOCK_IPCM == qpmapBlkType) {
                ...
                SampleCopyFlagsMap2Memory(width, height, maxCuSize, qpmapBlkUnit, QpMapVirAddrHost, qpmapBlkType);
            }
        }

        /* copy host frame memory to device */
        ret = axclrtMemcpy((void *)stFrame.stUserFrame.stVFrame.u64PhyAddr[0],
                            host_mem,
                            pCml->BlkSize,
                            AXCL_MEMCPY_HOST_TO_DEVICE);
        if (AXCL_SUCC != ret) {
        ...
        s32Ret = AXCL_VENC_SendFrameEx(VeChn, &stFrame, syncType);
        if (AX_SUCCESS != s32Ret)
...
AX_VOID *SAMPLE_VENC_SendFrameDynResolutionProc(AX_VOID *arg)
{
    ...
    /* malloc on host and memcpy to device */
    AX_VOID *host_mem = NULL;
    ret = axclrtMallocHost(&host_mem, pstArg->frameSize);
    if (AX_SUCCESS != ret) {
        SAMPLE_LOG_ERR("host malloc err,size=%d.\n",pstArg->frameSize);
        goto EXIT;
    }
    memset(&stFrame, 0, sizeof(stFrame));
    ...
    while (pstArg->bSendFrmStart) {
        if ((pstArg->totalSendFrame + 1) == pstArg->dynAttrIdx) {
            if (pstArg->function) {
                ...
                if (NULL != host_mem) {
                    axclrtFreeHost(host_mem);
                    host_mem = NULL;
                    ret = axclrtMallocHost(&host_mem, pstArg->frameSize); //alloc new pic memory
                    if (AX_SUCCESS != ret) {
                        SAMPLE_LOG_ERR("host malloc err,size=%d.\n",pstArg->frameSize);
                        goto EXIT;
                    }
        ...
        /* get block from user pool */
        blkId = AXCL_POOL_GetBlock(pstArg->poolId, pstArg->blkSize, NULL);
        if (AX_INVALID_BLOCKID == blkId) {
            usleep(5000);
            continue;
        }
            ...
            if (bLoopEncode) {
                ...
                if (pstArg->stFbcInfo.fbcType) {
                    readSize = COMMON_VENC_ReadFbcFile(pstArg->fFileIn, pstArg->strideY, pstArg->height,
                                                       &pstArg->stFbcInfo, &stFrame.stVFrame, host_mem);
                } else {
                    /* read frame data from yuv file */
...
AX_VOID *SAMPLE_VENC_SendFrameJencMultiSliceProc(AX_VOID *arg)
{
    ...
    while (pstArg->bSendFrmStart) {
        ...
        readSz = fread((AX_VOID *)host_mem, 1, width * subHeight, fFileIn);
        if (readSz < (width * subHeight))
            SAMPLE_LOG_WARN("chn-%d: read size(%d) less than (%d).\n", readSz, (width * subHeight));

        axclrtMemcpy((AX_VOID *)stFrame.stVFrame.u64PhyAddr[0],
                    host_mem,
                    width * subHeight,
                    AXCL_MEMCPY_HOST_TO_DEVICE);
        ...
        if (AX_FORMAT_YUV420_PLANAR == eFmt) {
            ...
            readSz = fread((AX_VOID *)host_mem, 1, uvSizePerSubImage, fFileIn);
            if (readSz < uvSizePerSubImage)
                SAMPLE_LOG_WARN("chn-%d: read size(%d) less than (%d).\n", readSz, uvSizePerSubImage);

            axclrtMemcpy((AX_VOID *)stFrame.stVFrame.u64PhyAddr[1],
                        host_mem,
                        uvSizePerSubImage,
                        AXCL_MEMCPY_HOST_TO_DEVICE);
            // V
            ...
            axclrtMemcpy((AX_VOID *)stFrame.stVFrame.u64PhyAddr[2],
                        host_mem,
                        uvSizePerSubImage,
                        AXCL_MEMCPY_HOST_TO_DEVICE);
...
AX_VOID *SAMPLE_VENC_GetStreamProc(AX_VOID *arg)
{
    ...
    if (NULL == host_mem) {
        SAMPLE_LOG_ERR("malloc host memory err.\n");
        goto EXIT;
    }
    ...
    while (pstArg->bGetStrmStart) {
        ...
        if (AX_SUCCESS == s32Ret) {
            ...
            if (AXCL_SUCC != ret) {
                SAMPLE_LOG_ERR("copy stream from device to host.\n");
                goto EXIT;
            }
