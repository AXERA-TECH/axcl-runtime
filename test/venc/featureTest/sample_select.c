...
static AX_VOID *SAMPLE_VENC_SltGetStreamProc(AX_VOID *arg)
{
    ...
    AX_S32 maxFd = 0;
    AX_S32 VencFd[MAX_VENC_CHN_NUM];
    fd_set read_fds;
    AX_U64 u64Pts = 0;
    ...
    while (pstArg->bGetStrmStart) {
        ...
        if (s32Ret < 0) {
            ...
            for (AX_S32 idx = startChn; idx < pCml->chnNum; idx += 2) {
                ...
                s32Ret = AXCL_VENC_GetStream(idx, &stStream, 0);
                if (AX_SUCCESS == s32Ret) {
                    ...
                    s32Ret = AXCL_VENC_ReleaseStream(idx, &stStream);
                    if (AX_SUCCESS != s32Ret) {
