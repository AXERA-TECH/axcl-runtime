...
static AX_VOID *SAMPLE_VENC_SendFrame_ResetChn_Proc(AX_VOID *arg)
{
    AX_BOOL bLoopEncode;
    AX_U32 encFrmNum;
    ...
    while (pstArg->bSendFrmStart) {
        ...
        blkId = AXCL_POOL_GetBlock(poolId, frameSize, NULL);
...
static AX_S32 SAMPLE_VENC_StartSendFrame_ResetChn(SAMPLE_VENC_SENDFRAME_PARA_T *pstArg)
{
    ...
    s32Ret = pthread_create(&pstArg->sendFrmPid, 0, SAMPLE_VENC_SendFrame_ResetChn_Proc, (AX_VOID *)pstArg);
