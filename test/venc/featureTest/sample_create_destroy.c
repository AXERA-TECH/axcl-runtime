...
#define SAMPLE_GET_STREAM_TIMEOUT_MS     (40 * 1000)
...
AX_S32 SAMPLE_VENC_SendFrame(AX_VOID *arg)
{
    ...
    ret = axclrtMallocHost(&host_mem, blkSize);
    if (AX_SUCCESS != ret) {
...
AX_S32 SAMPLE_VENC_GetRlsStream(AX_VOID *arg)
{
    ...
    s32Ret = AXCL_VENC_GetStream(VeChn, &stStream, SAMPLE_GET_STREAM_TIMEOUT_MS);
    if (AX_SUCCESS == s32Ret) {
...
AX_VOID *SAMPLE_VENC_SendGetProc(AX_VOID *arg)
{
    ...
    while (!gLoopExit) {
        ...
        for (AX_S32 encNum = 0; encNum < SAMPLE_ENCODE_NUM; encNum++) {
            ...
            s32Ret = AXCL_VENC_StartRecvFrame(chnNum, &stRecvParam);
            if (AX_SUCCESS != s32Ret) {
            ...
            s32Ret = AXCL_VENC_StopRecvFrame(chnNum);
            if (AX_SUCCESS != s32Ret) {
...
FREE:
    ...
    for (chnIdx = 0; chnIdx < chnNum; chnIdx++) {
        AXCL_VENC_StopRecvFrame(chnIdx);
