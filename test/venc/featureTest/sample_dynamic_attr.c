...
AX_S32 SampleTestBitrateJamStrategy(AX_S32 VeChn, AX_VOID *handle)
{
    ...
    s32Ret = AXCL_VENC_GetRateJamStrategy(VeChn, &stRateJamParam);
    if (s32Ret) {
    ...
    s32Ret = AXCL_VENC_SetRateJamStrategy(VeChn, &stRateJamParam);
    if (s32Ret) {
        SAMPLE_LOG_ERR("AX_VENC_SetRateJamStrategy error!\n");
        return -1;
    }

    s32Ret = AXCL_VENC_GetRateJamStrategy(VeChn, &stGetRateJamParam);
    if (s32Ret) {
        SAMPLE_LOG_ERR("AX_VENC_GetRateJamStrategy error!\n");
        return -1;
    }
...
AX_S32 SampleTestSuperFrameStrategy(AX_S32 VeChn, AX_VOID *handle)
{
    ...
    s32Ret = AXCL_VENC_GetSuperFrameStrategy(VeChn, &stSupFrm);
    if (AX_SUCCESS != s32Ret) {
    ...
    s32Ret = AXCL_VENC_SetSuperFrameStrategy(VeChn, &stSupFrm);
    if (AX_SUCCESS != s32Ret) {
        SAMPLE_LOG_ERR("VeChn:%d AX_VENC_SetSuperFrameStrategy error!\n", VeChn);
        return -1;
    }

    s32Ret = AXCL_VENC_GetSuperFrameStrategy(VeChn, &stSupFrmGet);
    if (AX_SUCCESS != s32Ret) {
        SAMPLE_LOG_ERR("VeChn:%d AX_VENC_GetSuperFrameStrategy error!\n", VeChn);
        return -1;
    }
...
AX_S32 SampleTestBitRate(AX_S32 VeChn, AX_VOID *handle)
{
    ...
    s32Ret = AXCL_VENC_GetRcParam(VeChn, &stRcParam);
    if (s32Ret) {
    ...
    s32Ret = AXCL_VENC_SetRcParam(VeChn, &stRcParam);
    if (s32Ret) {
...
AX_S32 SampleTestQpMap(AX_S32 VeChn, AX_VOID *handle)
{
    AX_S32 s32Ret = -1;
    AX_VENC_RC_PARAM_T stRcParam;
    SAMPLE_VENC_SENDFRAME_PARA_T *pCml = (SAMPLE_VENC_SENDFRAME_PARA_T *)handle;
    ...
    case PT_H264:
        if (stRcParam.enRcMode != AX_VENC_RC_MODE_H264CBR) {
            ...
            if (stRcParam.stH264Cbr.stQpmapInfo.enQpmapBlockType == AX_VENC_QPMAP_BLOCK_IPCM)
                stRcParam.stH264Cbr.stQpmapInfo.enQpmapBlockType = AX_VENC_QPMAP_BLOCK_SKIP;
            else
                stRcParam.stH264Cbr.stQpmapInfo.enQpmapBlockType = AX_VENC_QPMAP_BLOCK_IPCM;
            SAMPLE_LOG("change block type %d\n", stRcParam.stH264Cbr.stQpmapInfo.enQpmapBlockType);
        }
        break;
    case PT_H265:
        if (stRcParam.enRcMode != AX_VENC_RC_MODE_H265CBR) {
            ...
            if (stRcParam.stH265Cbr.stQpmapInfo.enQpmapBlockType == AX_VENC_QPMAP_BLOCK_IPCM)
                stRcParam.stH265Cbr.stQpmapInfo.enQpmapBlockType = AX_VENC_QPMAP_BLOCK_SKIP;
            else
                stRcParam.stH265Cbr.stQpmapInfo.enQpmapBlockType = AX_VENC_QPMAP_BLOCK_IPCM;
            SAMPLE_LOG("change block type %d\n", stRcParam.stH265Cbr.stQpmapInfo.enQpmapBlockType);
        }
        break;
    default:
        break;
...
AX_S32 SampleTestFrameRate(AX_S32 VeChn, AX_VOID *handle)
{
    ...
    if (PT_H265 == pCml->enType) {
        s32Ret = AXCL_VENC_RequestIDR(VeChn, pCml->bInsertIDR);
        if (AX_SUCCESS != s32Ret) {
...
AX_S32 SampleTestGop(AX_S32 VeChn, AX_VOID *handle)
{
    ...
    case AX_VENC_RC_MODE_H264CBR:
        stRcParam.stH264Cbr.u32Gop = pCml->gopLen + 6;
        break;
    case AX_VENC_RC_MODE_H264VBR:
        stRcParam.stH264Vbr.u32Gop = pCml->gopLen + 6;
        break;
    case AX_VENC_RC_MODE_H264AVBR:
        stRcParam.stH264AVbr.u32Gop = pCml->gopLen + 6;
        break;
    case AX_VENC_RC_MODE_H264FIXQP:
        stRcParam.stH264FixQp.u32Gop = pCml->gopLen + 6;
        break;
    case AX_VENC_RC_MODE_H265CBR:
        stRcParam.stH265Cbr.u32Gop = pCml->gopLen + 6;
        break;
    case AX_VENC_RC_MODE_H265VBR:
        stRcParam.stH265Vbr.u32Gop = pCml->gopLen + 6;
        break;
    case AX_VENC_RC_MODE_H265AVBR:
        stRcParam.stH265AVbr.u32Gop = pCml->gopLen + 6;
        break;
    case AX_VENC_RC_MODE_H265FIXQP:
        stRcParam.stH265FixQp.u32Gop = pCml->gopLen + 6;
        break;
...
static void SampleInfoDynRcModeDebug(AX_S32 VencChn, AX_PAYLOAD_TYPE_E enType, AX_VENC_RC_PARAM_T *pStRcParam)
{
    ...
    if (enType == PT_H264)
        SAMPLE_LOG_DEBUG("VencChn %d, enRcMode %d, u32Gop %d, u32IQp %d, u32PQp %d, u32BQp "
                         "%d\n",
...
AX_S32 SampleDynRcMode(AX_S32 VencChn, AX_VOID *handle)
{
    ...
    s32Ret = AXCL_VENC_GetRcParam(VencChn, &stRcParam);
    if (AX_SUCCESS != s32Ret) {
        SAMPLE_LOG_ERR("AX_VENC_GetRcParam:%d failed! ret=0x%x\n", VencChn, s32Ret);
        return -1;
    }

    if (-1 != stRcParam.s32FirstFrameStartQp) {
        stRcParam.s32FirstFrameStartQp = CLIP3(pstArg->qpMinI + 2, pstArg->qpMaxI - 2, stRcParam.s32FirstFrameStartQp);
    }
    ...
    if (SAMPLE_RC_FIXQP == pstArg->rcModeNew) {
        ...
        if (pstArg->enType == PT_H264) {
            ...
            stH265Cbr.u32IdrQpDeltaRange = 5;
        ...
        if (pstArg->enType == PT_H264) {
            ...
            stH264Vbr.u32IdrQpDeltaRange = 5;
            ...
            stH265Vbr.u32IdrQpDeltaRange = 5;
            if (pstArg->vq != AX_VENC_VBR_QUALITY_LEVEL_INV) {
                if (pstArg->vq < AX_VENC_VBR_QUALITY_LEVEL_8)
        ...
        if (pstArg->enType == PT_H264) {
            ...
            stH264Avbr.u32IdrQpDeltaRange = 5;
            ...
            stH265Avbr.u32IdrQpDeltaRange = 5;
        ...
        if (pstArg->enType == PT_H264) {
            ...
            stH264Cvbr.u32IdrQpDeltaRange = 5;
            ...
            stH265Cvbr.u32IdrQpDeltaRange = 5;
    ...
    s32Ret = AXCL_VENC_SetRcParam(VencChn, &stRcParam);
    if (AX_SUCCESS != s32Ret) {
...
AX_S32 SampleDynVui(AX_S32 VencChn, AX_VOID *handle)
{
    ...
    s32Ret = AXCL_VENC_GetVuiParam(VencChn, &stVui);
    if (AX_SUCCESS != s32Ret) {
    ...
    s32Ret = AXCL_VENC_SetVuiParam(VencChn, &stVui);
    if (AX_SUCCESS != s32Ret) {
...
AX_S32 SampleVirIntraInterval(AX_S32 VencChn, AX_VOID *handle)
{
    ...
    s32Ret = AXCL_VENC_GetChnAttr(VencChn, &stChnAttr);
    if (AX_SUCCESS != s32Ret) {
    ...
    s32Ret = AXCL_VENC_SetChnAttr(VencChn, &stChnAttr);
    if (AX_SUCCESS != s32Ret) {
...
AX_S32 SampleIntraRefresh(AX_S32 VencChn, AX_VOID *handle)
{
    ...
    s32Ret = AXCL_VENC_GetIntraRefresh(VencChn, &stIntraRefresh);
    if (AX_SUCCESS != s32Ret) {
    ...
    s32Ret = AXCL_VENC_SetIntraRefresh(VencChn, &stIntraRefresh);
    if (AX_SUCCESS != s32Ret) {
...
AX_S32 SampleDynResolution(AX_S32 VencChn, AX_VOID *handle)
{
    ...
    SAMPLE_VENC_SENDFRAME_PARA_T *pstArg = NULL;
    AX_VENC_RECV_PIC_PARAM_T stRecvParam = {.s32RecvPicNum = -1};
    ...
    pstArg->poolId = AXCL_POOL_CreatePool(&stPoolConfig);
    ...
    s32Ret = AXCL_VENC_ResetChn(VeChn);
    if (AX_SUCCESS != s32Ret) {
...
AX_S32 SampleRequestIDR(AX_S32 VeChn, AX_VOID *handle)
{
    ...
    s32Ret = AXCL_VENC_RequestIDR(VeChn, pstArg->bInsertIDR);
    if (AX_SUCCESS != s32Ret) {
...
AX_S32 SampleSetUserData(AX_S32 VeChn, AX_VOID *handle)
{
    ...
    s32Ret = AXCL_VENC_SetUsrData(VeChn, &stUsrData);
    if (s32Ret != AX_SUCCESS) {
        SAMPLE_LOG_ERR("chn-%d: AX_VENC_InsertUserData error!\n", VeChn);
        goto exit;
    }

    memcpy(&stUsrDataGet, &stUsrData, sizeof(AX_VENC_USR_DATA_T));
    ...
    s32Ret = AXCL_VENC_GetUsrData(VeChn, &stUsrDataGet);
    if (s32Ret != AX_SUCCESS) {
        SAMPLE_LOG_ERR("chn-%d: AX_VENC_GetUsrData error!\n", VeChn);
        goto exit;
    }
...
exit:
    ...
    SAMPLE_LOG("leave set user data.\n");
    return s32Ret;
}
...
AX_S32 SampleTestSliceSplit(AX_S32 VeChn, AX_VOID *handle)
{
    ...
    s32Ret = AXCL_VENC_GetSliceSplit(VeChn, &stSliceSplit);
    if (AX_SUCCESS != s32Ret) {
    ...
    s32Ret = AXCL_VENC_SetSliceSplit(VeChn, &stSliceSplit);
    if (AX_SUCCESS != s32Ret) {
...
AX_S32 SampleTestResetChn(AX_S32 VeChn, AX_VOID *handle)
{
    ...
    s32Ret = AXCL_VENC_StartRecvFrame(VeChn, &stRecv);
    if (AX_SUCCESS != s32Ret) {
...
AX_S32 SampleTestJencSliceSplit(AX_S32 VeChn, AX_VOID *handle)
{
    AX_S32 s32Ret = AX_SUCCESS;
    SAMPLE_VENC_SENDFRAME_PARA_T *pstArg = NULL;
