...
AX_VOID *SAMPLE_VENC_SelectGetStreamProc(AX_VOID *arg)
{
    ...
    SAMPLE_VENC_SetThreadName("GetStream-%d", grpId);
    ...
    s32Ret = AXCL_VENC_SelectClearGrp(grpId);
    if (AX_SUCCESS != s32Ret) {
    ...
    for (AX_S32 i = startChn; i < pstArg->chnNum; i += 2) {
        s32Ret = AXCL_VENC_SelectGrpAddChn(grpId, i);
        if (AX_SUCCESS != s32Ret) {
    ...
    s32Ret = AXCL_VENC_SelectGrpQuery(grpId, pstGrpParams);
    if (AX_SUCCESS != s32Ret) {
    ...
    while (pstArg->bGetStrmStart) {
        s32Ret = AXCL_VENC_SelectGrp(grpId, pstChnStrmState, -1);
        if (AX_SUCCESS != s32Ret) {
        ...
        for (AX_S32 idx = 0; idx < pstChnStrmState->u32TotalChnNum; idx++) {
            ...
            s32Ret = AXCL_VENC_GetStream(VeChn, &stStream, 0);
            if (AX_SUCCESS == s32Ret) {
                ...
                if (PT_JPEG == enType) {
                    s32Ret = COMMON_VENC_SaveJpegFile(VeChn, testId, totalGetStream, host_mem,
                    ...
                    SAMPLE_VENC_FWRITE(host_mem, 1, stStream.stPack.u32Len, gStrmFile[VeChn],
                                       pstArg->bSaveStrm);
