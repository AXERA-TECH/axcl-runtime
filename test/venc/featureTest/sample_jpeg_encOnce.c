...
static AX_S32 SampleJpegOnceOutBufCfg(AX_JPEG_ENCODE_ONCE_PARAMS_T *pStJpegEncodeOnceParam, AX_U32 frameSize)
{
    ...
    s32Ret = AXCL_SYS_MemAllocCached(&phyBuff, &virBuff, frameSize, 0, (AX_S8 *)"JPEG_ONCE_STREAM");
    if (s32Ret) {
        SAMPLE_LOG_ERR("alloc mem err, size(%d).\n", frameSize);
        return -1;
    }
...
static AX_S32 SampleJpegOnceReadFbcFile(FILE *pFileIn, AX_VIDEO_FRAME_T *pstFrame, SAMPLE_VENC_FBC_INFO_T *pFbc, AX_VOID *pVaddr)
{
    ...
    if (VENC_FBC_LOSSLESS == pFbc->fbcType) {
        ...
        readSize = fread(pVaddr, 1, lumaData, pFileIn);
        ...
        readSize += fread((pVaddr+lumaData), 1, chromaData, pFileIn);
                                          ...
                                          AX_JPEG_ENCODE_ONCE_PARAMS_T *pstOnceParams)
{
    ...
    s32Ret = AXCL_SYS_MemAlloc(&u64ThumbPhyAddr, &pThumbVirtAddr, u32ThumbSize, 0x1000, (AX_S8 *)"jenc_thumb_jpg");
    if (AX_SUCCESS != s32Ret) {
        SAMPLE_LOG_ERR("AX_VENC_JpegGetThumbnail failed, s32Ret=0x%x\n", s32Ret);
        return -1;
    }

    s32Ret = AXCL_VENC_JpegGetThumbnail(pstOnceParams->pu8Addr, pThumbVirtAddr, &u32ThumbSize);
    if (AX_SUCCESS != s32Ret) {
        SAMPLE_LOG_ERR("AX_VENC_JpegGetThumbnail failed, s32Ret=0x%x\n", s32Ret);
        goto exit;
    }
    ...
    if (pCmdl->bSaveStrm) {
        ...
        ret = axclrtMemcpy(host_mem, (void *)u64ThumbPhyAddr, u32ThumbSize, AXCL_MEMCPY_DEVICE_TO_HOST);
        if (AXCL_SUCC != ret) {
            SAMPLE_LOG_ERR("copy stream from device to host.\n");
            goto exit;
        }
        fwrite(host_mem, 1, u32ThumbSize, pStrm);
        fflush(pStrm);
        fclose(pStrm);
        pStrm = NULL;
    }
...
exit:
    ...
    AXCL_SYS_MemFree(u64ThumbPhyAddr, pThumbVirtAddr);
...
static AX_S32 JpegEncodeOneFrame(SAMPLE_VENC_ENCODEONCEFRAME_PARA_T *pCmdl)
{
    ...
    AX_VOID *host_frm_mem = NULL;
    AX_VOID *host_strm_mem = NULL;
    axclError ret;
    ...
    axclrtMallocHost(&host_frm_mem, frameSize);
    host_strm_mem = malloc(frameSize);
    if (NULL == host_strm_mem) {
        SAMPLE_LOG_ERR("malloc host memory err.\n");
        goto EXIT;
    }
    ...
    while (1) {
        ...
        if (pCmdl->stFbcInfo.fbcType)
            readSize = SampleJpegOnceReadFbcFile(fFileIn, &stFrame.stVFrame, &pCmdl->stFbcInfo,(void *)host_frm_mem);
        else
            readSize = COMMON_VENC_ReadFile(fFileIn, input_width, stFrame.stVFrame.u32PicStride[0], input_height,
                                            picFormat, (void *)host_frm_mem);
            ...
            if (bLoopEncode) {
                ...
                if (pCmdl->stFbcInfo.fbcType)
                    ...
                    readSize = COMMON_VENC_ReadFile(fFileIn, input_width, stFrame.stVFrame.u32PicStride[0],
                                                    input_height, picFormat, (void *)host_frm_mem);
            } else {
        ...
        /* copy host frame memory to device */
        ret = axclrtMemcpy(host_strm_mem,
                            (void *)stJpegEncodeOnceParam.ulPhyAddr,
                            stJpegEncodeOnceParam.u32Len,
        ...
        s32Ret = COMMON_VENC_SaveJpegFile(0, testId, totalEncodedFrames, host_strm_mem,
...
EXIT:
    if (NULL != host_frm_mem) {
        axclrtFreeHost(host_frm_mem);
        host_frm_mem = NULL;
    }
    if (NULL != host_strm_mem) {
        free(host_strm_mem);
    ...
    if (stFrame.stVFrame.u64PhyAddr[0] && (0 != stFrame.stVFrame.u64VirAddr[0])) {
        AXCL_SYS_MemFree(stFrame.stVFrame.u64PhyAddr[0], (void *)stFrame.stVFrame.u64VirAddr[0]);
    }
...
AX_VOID *SAMPLE_VENC_EncodeOnceProc(AX_VOID *pstArg) {
    ...
    SAMPLE_VENC_CMD_PARA_T *pCml = (SAMPLE_VENC_CMD_PARA_T *)pArg->private;
