...
static AX_S32 SAMPLE_IVE_CCL_OutImageConvert(const AX_IVE_IMAGE_T* pstSrcImg, AX_IVE_IMAGE_T* pstDstImg)
{
    AX_S32 s32Ret;
    if (pstSrcImg == NULL && pstDstImg == NULL) {
        SAMPLE_IVE_PRT("CCL 's pstSrcImg or pstDstImg is NULL\n");
        return AX_FAILURE;
    }

    AX_U32 size = ALIGN_UP(pstSrcImg->u32Width * pstSrcImg->u32Height * 24 / 8, 8);
    AX_U32 memcpysize = ALIGN_UP(pstSrcImg->u32Width * pstSrcImg->u32Height * 32 / 8, 8);

    s32Ret = axclrtMemcpy((AX_VOID *)pstSrcImg->au64VirAddr[0], (AX_VOID *)pstSrcImg->au64PhyAddr[0], memcpysize, AXCL_MEMCPY_DEVICE_TO_HOST);
    if (s32Ret) {
         SAMPLE_IVE_PRT("memcpy device memory %p to host memory %p fail, ret = 0x%x\n",
                        (AX_VOID *)pstSrcImg->au64PhyAddr[0], (AX_VOID *)pstSrcImg->au64VirAddr[0], s32Ret);
         return s32Ret;
    }
...
static AX_S32 SAMPLE_IVE_CCL_GetBlobFromDevice(AX_IVE_MEM_INFO_T *pstBlobMem)
{
    AX_S32 s32Ret;
    if(pstBlobMem == NULL) {
        SAMPLE_IVE_PRT("BlobMem is NULL\n");
        return AX_FAILURE;
    }
    if (pstBlobMem->u64PhyAddr == 0 || pstBlobMem->u64VirAddr == 0) {
        SAMPLE_IVE_PRT("BlobMem's PhyAddr or VirAddr is NULL\n");
        return AX_FAILURE;
    }

    s32Ret = axclrtMemcpy((AX_VOID *)pstBlobMem->u64VirAddr, (AX_VOID *)pstBlobMem->u64PhyAddr, pstBlobMem->u32Size, AXCL_MEMCPY_DEVICE_TO_HOST);
    if (s32Ret) {
         SAMPLE_IVE_PRT("memcpy device memory %p to host memory %p fail, ret = 0x%x\n",
                        (AX_VOID *)pstBlobMem->u64PhyAddr, (AX_VOID *)pstBlobMem->u64VirAddr, s32Ret);
         return s32Ret;
    }
    return AX_SUCCESS;
}
...
static AX_S32 SAMPLE_IVE_TestCclProc(TEST_CCL_T* pstTestCcl, AX_CHAR *pchParamsList)
{
    ...
    s32Ret = AXCL_IVE_CCL(&IveHandle, &pstTestCcl->stSrc, &pstTestCcl->stDst, &pstTestCcl->stBlob, &pstTestCcl->stCclCtrl, bInstant);
    if (AX_SUCCESS != s32Ret) {
    ...
    s32Ret = SAMPLE_IVE_CCL_GetBlobFromDevice(&pstTestCcl->stBlob);
    if (AX_SUCCESS != s32Ret) {
        SAMPLE_IVE_PRT("Get BlobInfo from device fail!\n");
        return s32Ret;
    }
    ...
    if (g_bSave) {
        ...
        if (u32BlobCnt > 0) {
            ...
            for (AX_S32 i = 0; i < 6; i++) {
                ...
                s32Ret = SAMPLE_COMM_IVE_CreateMemInfoHost(&stDumpStat[i], u32Size);
                if (AX_SUCCESS != s32Ret) {
            ...
            for (AX_S32 i = 0; i < 6; i++) {
                s32Ret = SAMPLE_COMM_IVE_WriteMemInfoFileHost(&stDumpStat[i], pFpDumpStat[i]);
            }
            if (AX_SUCCESS != s32Ret) {
