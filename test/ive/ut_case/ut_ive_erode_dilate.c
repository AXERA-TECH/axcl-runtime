...
static AX_S32 SAMPLE_IVE_TestEDCalcProc(TEST_ED_CALC_T* pstTestEDCalce, AX_U32 u32Mode, AX_CHAR *pchParamsList)
{
    ...
    if (u32Mode == 0) {
        s32Ret = AXCL_IVE_Erode(&IveHandle, &pstTestEDCalce->stSrc, &pstTestEDCalce->stDst, &stErodeCtrl, bInstant);
        if (AX_SUCCESS != s32Ret){
            SAMPLE_IVE_PRT("Error(%#x),AXCL_IVE_Erode failed!\n",s32Ret);
            return s32Ret;
        }
    } else {
        s32Ret = AXCL_IVE_Dilate(&IveHandle, &pstTestEDCalce->stSrc, &pstTestEDCalce->stDst, &stDilateCtrl, bInstant);
        if (AX_SUCCESS != s32Ret) {
            SAMPLE_IVE_PRT("Error(%#x),AXCL_IVE_Dilate failed!\n",s32Ret);
            return s32Ret;
        }
    }
