...
static AX_S32 SAMPLE_IVE_TestEdgeDetectionProc(TEST_EDGE_DETECTION_T* pstTestEdgeDetection, AX_U32 u32Mode, AX_CHAR *pchParamsList)
{
    ...
    if (u32Mode == 0) {
        s32Ret = AXCL_IVE_CannyHysEdge(&IveHandle, &pstTestEdgeDetection->stSrc1, &pstTestEdgeDetection->stSrc2, &pstTestEdgeDetection->stDst, &pstTestEdgeDetection->stHysEdgeCtrl, bInstant);
        if (AX_SUCCESS != s32Ret) {
            SAMPLE_IVE_PRT("Error(%#x),AX_IVE_CannyHysEdge failed!\n",s32Ret);
            return s32Ret;
        }
    } else {
        s32Ret = AXCL_IVE_CannyEdge(&IveHandle, &pstTestEdgeDetection->stSrc1, &pstTestEdgeDetection->stDst, &pstTestEdgeDetection->stCannyEdgeCtrl, bInstant);
        if (AX_SUCCESS != s32Ret) {
