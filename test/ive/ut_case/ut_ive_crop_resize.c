...
static AX_S32 SAMPLE_IVE_TestCropResizeProc(TEST_CROP_RESIZE_T* pstTestCropResize, AX_U32 u32Mode, AX_U32 u32Engine)
{
    ...
    case 0:
        ...
        s32Ret = AXCL_IVE_CropImage(&IveHandle, &pstTestCropResize->stSrc, pstDst, pstDstBoxs, &pstTestCropResize->stCropImageCtrl, u32Engine, bInstant);
        if (AX_SUCCESS != s32Ret) {
    ...
    case 1:
        u64StartTime = SAMPLE_COMM_IVE_GetTime_US();
        s32Ret = AXCL_IVE_CropResize(&IveHandle, &pstTestCropResize->stSrc, pstDst, pstDstBoxs, &pstTestCropResize->stCropResizeCtrl, u32Engine, bInstant);
        if (AX_SUCCESS != s32Ret) {
    ...
    case 2:
          //splite y and uv
        s32Ret = axclrtMemcpy((AX_VOID *)pstTestCropResize->stSrcY.au64PhyAddr[0], (AX_VOID *)pstTestCropResize->stSrc.au64VirAddr[0], pstTestCropResize->stSrcY.au32Stride[0] * pstTestCropResize->stSrcY.u32Height, AXCL_MEMCPY_HOST_TO_DEVICE);
        if (s32Ret) {
             SAMPLE_IVE_PRT("memcpy host memory %p to device memory %p fail, ret = 0x%x\n",
                            (AX_VOID *)pstTestCropResize->stSrc.au64VirAddr[0], (AX_VOID *)pstTestCropResize->stSrcY.au64PhyAddr[0], s32Ret);
             return s32Ret;
        }
        s32Ret = axclrtMemcpy((AX_VOID *)pstTestCropResize->stSrcUV.au64PhyAddr[0], (AX_VOID *)pstTestCropResize->stSrc.au64VirAddr[1], 2 * pstTestCropResize->stSrcUV.au32Stride[0] * pstTestCropResize->stSrcUV.u32Height, AXCL_MEMCPY_HOST_TO_DEVICE);
        if (s32Ret) {
             SAMPLE_IVE_PRT("memcpy host memory %p to device memory %p fail, ret = 0x%x\n",
                            (AX_VOID *)pstTestCropResize->stSrc.au64VirAddr[1], (AX_VOID *)pstTestCropResize->stSrcUV.au64PhyAddr[0], s32Ret);
             return s32Ret;
        }
        ...
        u64StartTime = SAMPLE_COMM_IVE_GetTime_US();
        s32Ret = AXCL_IVE_CropResizeForSplitYUV(&IveHandle, &pstTestCropResize->stSrcY, &pstTestCropResize->stSrcUV, pstDstY, pstDstUV, pstDstBoxs,&pstTestCropResize->stCropResizeCtrl, u32Engine, bInstant);
        if (AX_SUCCESS != s32Ret) {
    ...
    if (g_bSave) {
        for (AX_S32 i = 0; i < u32OutputNum; i++) {
            if (u32Mode == 2) {
                //concat y and uv
                AX_U32 imgsize = pstTestCropResize->stDstY[i].au32Stride[0] * pstTestCropResize->stDstY[i].u32Height;
                s32Ret = axclrtMemcpy((AX_VOID *)pstTestCropResize->stDst[i].au64VirAddr[0], (AX_VOID *)pstTestCropResize->stDstY[i].au64PhyAddr[0], imgsize, AXCL_MEMCPY_DEVICE_TO_HOST);
                if (s32Ret) {
                     SAMPLE_IVE_PRT("memcpy device memory %p to host memory %p fail, ret = 0x%x\n",
                                    (AX_VOID *)pstTestCropResize->stDst[i].au64PhyAddr[0], (AX_VOID *)pstTestCropResize->stDstY[i].au64VirAddr[0], s32Ret);
                     return s32Ret;
                }
                imgsize = 2 * pstTestCropResize->stDstUV[i].au32Stride[0] * pstTestCropResize->stDstUV[i].u32Height;
                s32Ret = axclrtMemcpy((AX_VOID *)pstTestCropResize->stDst[i].au64VirAddr[1], (AX_VOID *)pstTestCropResize->stDstUV[i].au64PhyAddr[0], imgsize, AXCL_MEMCPY_DEVICE_TO_HOST);
                if (s32Ret) {
                     SAMPLE_IVE_PRT("memcpy device memory %p to host memory %p fail, ret = 0x%x\n",
                                    (AX_VOID *)pstTestCropResize->stDstUV[i].au64PhyAddr[0], (AX_VOID *)pstTestCropResize->stDst[i].au64VirAddr[1], s32Ret);
                     return s32Ret;
                }
            }
            ...
            if (u32Mode == 0 && u32Engine == AX_IVE_ENGINE_IVE){
                ...
                if (u32Mode == 2)
                    s32Ret = SAMPLE_COMM_IVE_WriteFile_WithGlbImgFmtHost(&pstTestCropResize->stDst[i], pstTestCropResize->pFpDst[i]);
                else
