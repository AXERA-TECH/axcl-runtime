...
static AX_S32 SAMPLE_IVE_TestGmmProc(TEST_GMM_T* pstTestGmm, AX_U32 u32Mode, AX_CHAR *pchParamsList)
{
    ...
    bInstant = AX_FALSE;
    if (u32Mode == 0) {
        s32Ret = AXCL_IVE_GMM(&IveHandle, &pstTestGmm->stSrc, &pstTestGmm->stFg,
            &pstTestGmm->stBg, &pstTestGmm->stModel, &pstTestGmm->stGmmCtrl, bInstant);
    } else if (u32Mode == 1) {
        s32Ret = AXCL_IVE_GMM2(&IveHandle, &pstTestGmm->stSrc, &pstTestGmm->stFg,
            &pstTestGmm->stBg, &pstTestGmm->stModel, &pstTestGmm->stGmm2Ctrl, bInstant);
    }
    if (AX_SUCCESS != s32Ret) {
        SAMPLE_IVE_PRT("Error(%#x),AXCL_IVE_GMM/GMM2 failed!\n",s32Ret);
        return s32Ret;
    }
