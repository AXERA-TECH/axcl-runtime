...
static AX_S32 SAMPLE_IVE_TestCropResize2_ParseParams(TEST_CROP_RESIZE2_T* pstTestCropResize2, AX_CHAR *pchParamsList)
{
    ...
    if (!fp) {
        ...
        if (buf) {
            ...
            if (file_size != len) {
                fclose(fp);
                SAMPLE_IVE_PRT("Error:read json failed!\n");
                return AX_FAILURE;
            }
...
static AX_S32 SAMPLE_IVE_CreateImage_Y_UV(AX_IVE_IMAGE_T* pstImg, AX_S32 enType, AX_U32 u32Width, AX_U32 u32Height, AX_BOOL bIsUV)
{
    ...
    if (bIsUV)
        u32Size = 2 * u32Size;
    s32Ret = axclrtMalloc((AX_VOID **)&pstImg->au64PhyAddr[0], u32Size, AXCL_MEM_MALLOC_NORMAL_ONLY);
...
static AX_S32 SAMPLE_IVE_TestCropResize2Proc(TEST_CROP_RESIZE2_T* pstTestCropResize2, AX_U32 u32Mode, AX_U32 u32Engine)
{
    ...
    case 0:
        ...
        s32Ret = AXCL_IVE_CropResize2(&IveHandle, &pstTestCropResize2->stSrc, pstDst, pstSrcBoxs, pstDstBoxs, &pstTestCropResize2->stCropResize2Ctrl, u32Engine, bInstant);
        if (AX_SUCCESS != s32Ret) {
    ...
    case 1:
        //splite y and uv
        s32Ret = axclrtMemcpy((AX_VOID *)pstTestCropResize2->stSrcY.au64PhyAddr[0], (AX_VOID *)pstTestCropResize2->stSrc.au64VirAddr[0], pstTestCropResize2->stSrcY.au32Stride[0] * pstTestCropResize2->stSrcY.u32Height, AXCL_MEMCPY_HOST_TO_DEVICE);
        if (s32Ret) {
             SAMPLE_IVE_PRT("memcpy host memory %p to device memory %p fail, ret = 0x%x\n",
                            (AX_VOID *)pstTestCropResize2->stSrc.au64VirAddr[0], (AX_VOID *)pstTestCropResize2->stSrcY.au64PhyAddr[0], s32Ret);
             return s32Ret;
        }
        s32Ret = axclrtMemcpy((AX_VOID *)pstTestCropResize2->stSrcUV.au64PhyAddr[0], (AX_VOID *)pstTestCropResize2->stSrc.au64VirAddr[1], 2 * pstTestCropResize2->stSrcUV.au32Stride[0] * pstTestCropResize2->stSrcUV.u32Height, AXCL_MEMCPY_HOST_TO_DEVICE);
        if (s32Ret) {
             SAMPLE_IVE_PRT("memcpy host memory %p to device memory %p fail, ret = 0x%x\n",
                            (AX_VOID *)pstTestCropResize2->stSrc.au64VirAddr[1], (AX_VOID *)pstTestCropResize2->stSrcUV.au64PhyAddr[0], s32Ret);
             return s32Ret;
        }
        ...
        for (AX_S32 i = 0; i < u32OutputNum; i++) {
            ...
            s32Ret = axclrtMemcpy((AX_VOID *)pstTestCropResize2->stDstY[i].au64PhyAddr[0], (AX_VOID *)pstTestCropResize2->stDst[i].au64VirAddr[0], pstTestCropResize2->stDstY[i].au32Stride[0] * pstTestCropResize2->stDstY[i].u32Height, AXCL_MEMCPY_HOST_TO_DEVICE);
            if (s32Ret) {
                 SAMPLE_IVE_PRT("memcpy host memory %p to device memory %p fail, ret = 0x%x\n",
                                (AX_VOID *)pstTestCropResize2->stDst[i].au64VirAddr[0], (AX_VOID *)pstTestCropResize2->stDstY[i].au64PhyAddr[0], s32Ret);
                 return s32Ret;
            }
            s32Ret = axclrtMemcpy((AX_VOID *)pstTestCropResize2->stDstUV[i].au64PhyAddr[0], (AX_VOID *)pstTestCropResize2->stDst[i].au64VirAddr[1], 2 * pstTestCropResize2->stDstUV[i].au32Stride[0] * pstTestCropResize2->stDstUV[i].u32Height, AXCL_MEMCPY_HOST_TO_DEVICE);
            if (s32Ret) {
                 SAMPLE_IVE_PRT("memcpy host memory %p to device memory %p fail, ret = 0x%x\n",
                                (AX_VOID *)pstTestCropResize2->stDst[i].au64VirAddr[1], (AX_VOID *)pstTestCropResize2->stDstUV[i].au64PhyAddr[0], s32Ret);
                 return s32Ret;
            }
        ...
        u64StartTime = SAMPLE_COMM_IVE_GetTime_US();
        s32Ret = AXCL_IVE_CropResize2ForSplitYUV(&IveHandle, &pstTestCropResize2->stSrcY, &pstTestCropResize2->stSrcUV, pstDstY, pstDstUV, pstSrcBoxs, pstDstBoxs, &pstTestCropResize2->stCropResize2Ctrl, u32Engine, bInstant);
        if (AX_SUCCESS != s32Ret) {
    ...
    if (g_bSave) {
        for (AX_S32 i = 0; i < u32OutputNum; i++) {
            if (u32Mode == 1) {
                ...
                AX_U32 imgsize = pstTestCropResize2->stDstY[i].au32Stride[0] * pstTestCropResize2->stDstY[i].u32Height;
                s32Ret = axclrtMemcpy((AX_VOID *)pstTestCropResize2->stDst[i].au64VirAddr[0], (AX_VOID *)pstTestCropResize2->stDstY[i].au64PhyAddr[0], imgsize, AXCL_MEMCPY_DEVICE_TO_HOST);
                if (s32Ret) {
                     SAMPLE_IVE_PRT("memcpy device memory %p to host memory %p fail, ret = 0x%x\n",
                                    (AX_VOID *)pstTestCropResize2->stDst[i].au64PhyAddr[0], (AX_VOID *)pstTestCropResize2->stDstY[i].au64VirAddr[0], s32Ret);
                     return s32Ret;
                }
                imgsize = 2 * pstTestCropResize2->stDstUV[i].au32Stride[0] * pstTestCropResize2->stDstUV[i].u32Height;
                s32Ret = axclrtMemcpy((AX_VOID *)pstTestCropResize2->stDst[i].au64VirAddr[1], (AX_VOID *)pstTestCropResize2->stDstUV[i].au64PhyAddr[0], imgsize, AXCL_MEMCPY_DEVICE_TO_HOST);
                if (s32Ret) {
                     SAMPLE_IVE_PRT("memcpy device memory %p to host memory %p fail, ret = 0x%x\n",
                                    (AX_VOID *)pstTestCropResize2->stDstUV[i].au64PhyAddr[0], (AX_VOID *)pstTestCropResize2->stDst[i].au64VirAddr[1], s32Ret);
                     return s32Ret;
                }
            }
            s32Ret = SAMPLE_COMM_IVE_WriteFile_WithGlbImgFmtHost(&pstTestCropResize2->stDst[i], pstTestCropResize2->pFpDst[i]);
            if (AX_SUCCESS != s32Ret) {
