...
static AX_VOID SAMPLE_COMM_IVE_Hist_OutConvert(AX_U32 u32Mode, const AX_IVE_MEM_INFO_T *pstSrc, AX_IVE_MEM_INFO_T *pstDst)
{
    ...
    if (axclrtMemcpy((AX_VOID *)pstSrc->u64VirAddr, (AX_VOID *)pstSrc->u64PhyAddr, pstSrc->u32Size, AXCL_MEMCPY_DEVICE_TO_HOST)) {
        return;
    }
...
static AX_S32 SAMPLE_IVE_TestHistProc(TEST_HIST_T* pstTestHist, AX_U32 u32Mode, AX_CHAR *pchParamsList)
{
    ...
    if (u32Mode)
        s32Ret = AXCL_IVE_EqualizeHist(&IveHandle, &pstTestHist->stSrc, &pstTestHist->stDst, &pstTestHist->stEqualizeHistCtrl, bInstant);
    else
        s32Ret = AXCL_IVE_Hist(&IveHandle, &pstTestHist->stSrc, &pstTestHist->stDst, bInstant);
    if (AX_SUCCESS != s32Ret) {
        SAMPLE_IVE_PRT("Error(%#x),AXCL_IVE_Hist/EqualizeHist failed!\n",s32Ret);
        return s32Ret;
    }
    if (bInstant == AX_FALSE) {
        ...
        while (AX_ERR_IVE_QUERY_TIMEOUT == s32Ret) {
            usleep(1000*100);
            SAMPLE_IVE_PRT("AXCL_IVE_Query timeout, retry...\n");
            s32Ret = AXCL_IVE_Query(IveHandle, &bFinish, bBlock);
        }
        if (AX_SUCCESS != s32Ret) {
            SAMPLE_IVE_PRT("Error(%#x),AXCL_IVE_Query failed!\n",s32Ret);
            return s32Ret;
        }
    }
    ...
    if (g_bSave) {
        ...
        s32Ret = SAMPLE_COMM_IVE_WriteMemInfoFileHost(&pstTestHist->stDstSave, pstTestHist->pFpDstMemInfo);
        if (AX_SUCCESS != s32Ret) {
