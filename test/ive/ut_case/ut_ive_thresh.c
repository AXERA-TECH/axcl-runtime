...
static AX_S32 SAMPLE_IVE_TestThreshProc(TEST_THRESH_T* pstTestThresh, AX_CHAR *pchParamsList)
{
    ...
    s32Ret = AXCL_IVE_Thresh(&IveHandle, &pstTestThresh->stSrc, &pstTestThresh->stDst, &pstTestThresh->stThrCtrl, bInstant);
    if (AX_SUCCESS != s32Ret) {
        SAMPLE_IVE_PRT("Error(%#x),AXCL_IVE_Thresh failed!\n",s32Ret);
        return s32Ret;
    }
