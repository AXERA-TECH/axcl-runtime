...
static AX_S32 SAMPLE_IVE_TestFilterProc(TEST_FILTER_T* pstTestFilter, AX_CHAR *pchParamsList)
{
    ...
    s32Ret = AXCL_IVE_Filter(&IveHandle, &pstTestFilter->stSrc, &pstTestFilter->stDst, &pstTestFilter->stFltCtrl, bInstant);
    if (AX_SUCCESS != s32Ret) {
        SAMPLE_IVE_PRT("Error(%#x),AXCL_IVE_Filter failed!\n",s32Ret);
        return s32Ret;
    }
