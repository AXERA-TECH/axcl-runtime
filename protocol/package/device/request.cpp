...
struct axcl::pkg::device::request::impl {
    explicit impl() {
...
private:
    axcl::response::Payload m_payload;
    uint8_t* m_pkg = nullptr;
    size_t m_pkg_size = 0;
    uint8_t m_min_pkg[MIN_PKG_HOLD_SIZE];
};

axcl::pkg::device::request::request() : m_impl(new impl()) {
}

axcl::pkg::device::request::request(request&& other) noexcept : m_impl(std::exchange(other.m_impl, nullptr)) {
}

axcl::pkg::device::request& axcl::pkg::device::request::operator=(request&& other) noexcept {
    if (this != &other) {
        delete m_impl;
...
axcl::pkg::device::request::~request() {
    delete m_impl;
}

void axcl::pkg::device::request::set_env(const axcl::pkg::environment& env) const {
    m_impl->set_env(env);
}

void axcl::pkg::device::request::set_timestamp(uint64_t timestamp) const {
    m_impl->set_timestamp(timestamp);
}

void axcl::pkg::device::request::set_version(const axcl::pkg::version& v) const {
    m_impl->set_version(v);
}

uint64_t axcl::pkg::device::request::get_timestamp() const {
    return m_impl->get_timestamp();
}

uint64_t axcl::pkg::device::request::get_seq_num() const {
    return m_impl->get_seq_num();
}

axcl::pkg::version axcl::pkg::device::request::get_version() const {
    return m_impl->get_version();
}

axcl::pkg::environment axcl::pkg::device::request::get_env() const {
    return m_impl->get_env();
}

[[nodiscard]] void* axcl::pkg::device::request::select(const axcl::pkg::module& module) const {
    return m_impl->select(module);
}

[[nodiscard]] void* axcl::pkg::device::request::encode(const axcl::pkg::head& hd, uint32_t& size) const {
