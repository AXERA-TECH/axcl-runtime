...
struct axcl::pkg::device::vdec_response::impl {
...
private:
    axcl::native::module::vdec::Request* m_payload = {nullptr};
};

axcl::pkg::device::vdec_response::vdec_response() : m_impl(new impl()) {
}

axcl::pkg::device::vdec_response::vdec_response(vdec_response&& other) noexcept : m_impl(std::exchange(other.m_impl, nullptr)) {
}

axcl::pkg::device::vdec_response& axcl::pkg::device::vdec_response::operator=(vdec_response&& other) noexcept {
    if (this != &other) {
        delete m_impl;
...
axcl::pkg::device::vdec_response::~vdec_response() {
    delete m_impl;
}

[[nodiscard]] bool axcl::pkg::device::vdec_response::decode(const void* message, uint32_t size) const {
...
void* axcl::pkg::device::vdec_response::get_payload() const {
    return m_impl->get_payload();
}

axcl::pkg::native::VDEC_API axcl::pkg::device::vdec_response::get_api() const {
    return m_impl->get_api();
}

bool axcl::pkg::device::vdec_response::check_api(axcl::pkg::native::API api) {
    return m_impl->check_api(api.vdec);
}

size_t axcl::pkg::device::vdec_response::get_data_size(int index) {
    return m_impl->get_data_size(index);
}

bool axcl::pkg::device::vdec_response::get_data(int index, void* data, size_t size) {
    return m_impl->get_data(index, data, size);
}

const std::string& axcl::pkg::device::vdec_response::get_data(int index) {
