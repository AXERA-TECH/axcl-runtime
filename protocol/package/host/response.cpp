...
struct axcl::pkg::host::response::impl {
    ...
    [[nodiscard]] bool decode(const void* message, uint32_t size) {
        ...
        if (!m_payload.ParseFromArray((void*)((uint8_t*)message + PACKAGE_HEAD_SIZE), (int)(size - PACKAGE_HEAD_SIZE))) {
            LOG_MM_E(TAG, "host decode response message fail");
            return false;
        }
...
axcl::pkg::host::response::response() : m_impl(new impl()) {
}

axcl::pkg::host::response::response(response&& other) noexcept : m_impl(std::exchange(other.m_impl, nullptr)) {
}

axcl::pkg::host::response& axcl::pkg::host::response::operator=(response&& other) noexcept {
    if (this != &other) {
        delete m_impl;
...
axcl::pkg::host::response::~response() {
    delete m_impl;
}

[[nodiscard]] bool axcl::pkg::host::response::decode(const void* message, uint32_t size) const {
    return m_impl->decode(message, size);
}

uint64_t axcl::pkg::host::response::get_seq_num() const {
    return m_impl->get_seq_num();
}

axcl::pkg::version axcl::pkg::host::response::get_version() const {
    return m_impl->get_version();
}

axcl::pkg::environment axcl::pkg::host::response::get_env() const {
    return m_impl->get_env();
}

uint64_t axcl::pkg::host::response::get_timestamp() const {
    return m_impl->get_timestamp();
}

axcl::pkg::module axcl::pkg::host::response::get_module() const {
    return m_impl->get_module();
}

void* axcl::pkg::host::response::get_payload() const {
