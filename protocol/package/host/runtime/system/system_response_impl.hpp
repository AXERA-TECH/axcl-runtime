...
struct system_response::impl {
    [[nodiscard]] bool check_payload() {
        ...
        if (status) {
            ...
            if (!m_payload) {
                LOG_MM_E(TAG, "system response payload is nil");
            } else {
                LOG_MM_E(TAG, "check system response payload fail, has head {} has body {}", m_payload->has_head(), m_payload->has_body());
            }
        }
...
private:
    axcl::runtime::system::response::Payload *m_payload = {nullptr};
