...
struct axcl::pkg::host::native_response::impl {
    ...
    [[nodiscard]] bool select(axcl::response::Payload* payload) {
...
private:
    axcl::native::response::Payload* m_payload = {nullptr};
};

axcl::pkg::host::native_response::native_response() : m_impl(new impl()) {
}

axcl::pkg::host::native_response::native_response(native_response&& other) noexcept : m_impl(std::exchange(other.m_impl, nullptr)) {
}

axcl::pkg::host::native_response& axcl::pkg::host::native_response::operator=(native_response&& other) noexcept {
    if (this != &other) {
        delete m_impl;
...
axcl::pkg::host::native_response::~native_response() {
    delete m_impl;
}

[[nodiscard]] bool axcl::pkg::host::native_response::decode(const void* message, uint32_t size) const {
    if (!response::decode(message, size)) {
        return false;
    }

    m_arg_index = 0;
    return m_impl->select(static_cast<axcl::response::Payload*>(response::get_payload()));
}

axcl::pkg::native::module axcl::pkg::host::native_response::get_module() const {
    return m_impl->get_module();
}

void* axcl::pkg::host::native_response::get_payload() const {
    return m_impl->get_payload();
}

bool axcl::pkg::host::native_response::check_api(axcl::pkg::native::API api) {
    return false;
}

size_t axcl::pkg::host::native_response::get_data_size(int index) {
    return 0;
}

bool axcl::pkg::host::native_response::get_data(int index, void* data, size_t size) {
    return false;
}

const std::string& axcl::pkg::host::native_response::get_data(int index) {
