...
struct axcl::pkg::host::native_request::impl {
    impl() = delete;

    explicit impl(void* payload) : m_payload(static_cast<axcl::native::request::Payload*>(payload)) {
...
private:
    axcl::native::request::Payload* m_payload;
};

axcl::pkg::host::native_request::native_request()
    : m_args([this](const void* arg, size_t size) { return add_data(arg, size); }),
      m_impl(new impl(request::select(axcl::pkg::module::NATIVE))) {
}

axcl::pkg::host::native_request::native_request(native_request&& other) noexcept
    : request(std::move(other)),
      m_args([this](const void* arg, size_t size) { return add_data(arg, size); }),
      m_impl(std::exchange(other.m_impl, nullptr)) {
}

axcl::pkg::host::native_request& axcl::pkg::host::native_request::operator=(native_request&& other) noexcept {
    if (this != &other) {
        delete m_impl;
        m_impl = std::exchange(other.m_impl, nullptr);
    }
...
[[nodiscard]] void* axcl::pkg::host::native_request::select(const axcl::pkg::native::module& module) const {
    return m_impl->select(module);
}

bool axcl::pkg::host::native_request::add_data(const void* data, size_t size) {
    return false;
}