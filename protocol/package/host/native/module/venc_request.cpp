...
struct axcl::pkg::host::venc_request::impl {
    impl() = delete;

    explicit impl(void* payload) : m_payload(static_cast<axcl::native::module::venc::Request*>(payload)) {
    }
...
private:
    axcl::native::module::venc::Request* m_payload;
};

axcl::pkg::host::venc_request::venc_request()
    : m_impl(new impl(native_request::select(axcl::pkg::native::module::VENC))) {
}

axcl::pkg::host::venc_request::venc_request(venc_request&& other) noexcept
    : native_request(std::move(other)), m_impl(std::exchange(other.m_impl, nullptr)) {
}

axcl::pkg::host::venc_request& axcl::pkg::host::venc_request::operator=(venc_request&& other) noexcept {
    if (this != &other) {
        delete m_impl;
...
axcl::pkg::native::VENC_API axcl::pkg::host::venc_request::get_api() const {
    return m_impl->get_api();
}

bool axcl::pkg::host::venc_request::add_data(const void* data, size_t size) {
    return m_impl->add_data(data, size);
}

bool axcl::pkg::host::venc_request::set_api(axcl::pkg::native::VENC_API api) {
    return m_impl->set_api(api);
}

bool axcl::pkg::host::venc_request::serialize_gop_svc_t(const AX_VENC_GOP_SVC_T_T& svc, uint8_t*& buf, uint32_t& size) {
    uint32_t count = 0;
