...
struct axcl::pkg::host::venc_response::impl {
...
private:
    axcl::native::module::venc::Response* m_payload = {nullptr};
};

axcl::pkg::host::venc_response::venc_response() : m_impl(new impl()) {
}

axcl::pkg::host::venc_response::venc_response(venc_response&& other) noexcept : m_impl(std::exchange(other.m_impl, nullptr)) {
}

axcl::pkg::host::venc_response& axcl::pkg::host::venc_response::operator=(venc_response&& other) noexcept {
    if (this != &other) {
        delete m_impl;
...
axcl::pkg::host::venc_response::~venc_response() {
    delete m_impl;
}

[[nodiscard]] bool axcl::pkg::host::venc_response::decode(const void* message, uint32_t size) const {
...
void* axcl::pkg::host::venc_response::get_payload() const {
    return m_impl->get_payload();
}

axcl::pkg::native::VENC_API axcl::pkg::host::venc_response::get_api() const {
    return m_impl->get_api();
}

bool axcl::pkg::host::venc_response::check_api(axcl::pkg::native::API api) {
    return m_impl->check_api(api.venc);
}

size_t axcl::pkg::host::venc_response::get_data_size(int index) {
    return m_impl->get_data_size(index);
}

bool axcl::pkg::host::venc_response::get_data(int index, void* data, size_t size) {
    return m_impl->get_data(index, data, size);
}

const std::string& axcl::pkg::host::venc_response::get_data(int index) {
    return m_impl->get_data(index);
}

bool axcl::pkg::host::venc_response::deserialize_gop_svc_t(const uint8_t* buf, uint32_t size, AX_VENC_GOP_SVC_T_T& svc,
