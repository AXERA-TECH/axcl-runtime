...
struct axcl::pkg::host::ive_response::impl {
...
private:
    axcl::native::module::ive::Response* m_payload = {nullptr};
};

axcl::pkg::host::ive_response::ive_response() : m_impl(new impl()) {
}

axcl::pkg::host::ive_response::ive_response(ive_response&& other) noexcept : m_impl(std::exchange(other.m_impl, nullptr)) {
}

axcl::pkg::host::ive_response& axcl::pkg::host::ive_response::operator=(ive_response&& other) noexcept {
    if (this != &other) {
        delete m_impl;
...
axcl::pkg::host::ive_response::~ive_response() {
    delete m_impl;
}

[[nodiscard]] bool axcl::pkg::host::ive_response::decode(const void* message, uint32_t size) const {
...
void* axcl::pkg::host::ive_response::get_payload() const {
    return m_impl->get_payload();
}

axcl::pkg::native::IVE_API axcl::pkg::host::ive_response::get_api() const {
    return m_impl->get_api();
}

bool axcl::pkg::host::ive_response::check_api(axcl::pkg::native::API api) {
    return m_impl->check_api(api.ive);
}

size_t axcl::pkg::host::ive_response::get_data_size(int index) {
    return m_impl->get_data_size(index);
}

bool axcl::pkg::host::ive_response::get_data(int index, void* data, size_t size) {
    return m_impl->get_data(index, data, size);
}

const std::string& axcl::pkg::host::ive_response::get_data(int index) {
