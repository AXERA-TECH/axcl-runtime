...
Requires(post): gcc >= 9.4.0, make, patch, kernel-devel, kernel-headers
...
if [ "${DIRECT_COMPILE}" = "1" ]; then
    ...
    cp -rf %{_usrsrc}/%{module_name}-%{module_version}/out/axcl_linux_x86/ko/* %{_lib}/modules/$(uname -r)/extra/
fi
