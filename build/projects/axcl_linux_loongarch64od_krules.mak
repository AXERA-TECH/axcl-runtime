...
modules: $(KBUILD_MAKEFILE)
	$(MAKE) ARCH=loongarch CROSS_COMPILE=$(CROSS) KCFLAGS="$(KCFLAGS)" -C $(KERNEL_BUILD) M=$(SRC_PATH) $(EXT_FLAG) $@
#   WR: solve kernel 6.11.0-19 O=xxx compile issue
	@$(CP) $(SRC_PATH)/$(MODULE_NAME).ko $(KBUILD_DIR)/
	@$(CP) $(SRC_PATH)/*.symvers $(KBUILD_DIR)/
	$(MAKE) -C $(KERNEL_BUILD) M=$(SRC_PATH) clean
